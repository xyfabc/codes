...
typedef struct hiSAMPLE_VI_PARAM_S
{
    HI_S32 s32ViDevCnt;
    HI_S32 s32ViDevInterval;
    HI_S32 s32ViChnCnt ;
    HI_S32 s32ViChnInterval;
...
HI_S32 SAMPLE_COMM_VI_Start(SAMPLE_VI_MODE_E enViMode, VIDEO_NORM_E enNorm)
{
    ...
    for(i=0; i<stViParam.s32ViChnCnt; i++)
    {
        ...
        s32Ret = SAMPLE_COMM_VI_StartChn(ViChn, &stCapRect, &stMainTargetSize, enViMode);
        if (HI_SUCCESS != s32Ret)
        {
        ...
        if (HI_TRUE == SAMPLE_COMM_VI_IsHd(enViMode))
        {
            ...
            s32Ret = SAMPLE_COMM_VI_StartChn(ViChn_Sub, &stCapRect, &stSubTargetSize,enViMode);
            if (HI_SUCCESS != s32Ret)
            {
