...
{  
    ...
    unsigned int idx;
    unsigned int fd;
    unsigned int slave_address, reg_address, data;
    unsigned char val[2];
    ...
    if(argc<5)
    {
        printf("Use:\n%s /dev/i2c-x slave reg_addr data\n",argv[0]);
        return 0;
    }
    ...
    printf("i2c-test : slave = 0x%02x, reg_addr = 0x%02x, data = 0x%02x\n", slave_address, reg_address, data);
    ...
    if(ret < 0)
        ...
        printf("Reg: %02x val: %02x.  Success.\n",reg_address, data);
