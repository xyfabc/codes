...
HI_S32 SAMPLE_COMM_AUDIO_CfgAcodec(AIO_ATTR_S *pstAioAttr)
{
    ...
    if (!bCodecCfg)
    {
        printf("%s:SAMPLE_COMM_AUDIO_CfgAcodec failed, not config the right codec\n", __FUNCTION__);
        return HI_FALSE;
    }
...
void *SAMPLE_COMM_AUDIO_AiProc(void *parg)
{
    ...
    {     
        ...
        if (FD_ISSET(AiFd, &read_fds))
        {
            ...
            s32Ret = HI_MPI_AI_GetFrame(pstAiCtl->AiDev, pstAiCtl->AiChn, &stFrame, &stAecFrame, HI_FALSE);
            if (HI_SUCCESS != s32Ret )
            {
            ...
            {
                s32Ret = HI_MPI_AENC_SendFrame(pstAiCtl->AencChn, &stFrame, &stAecFrame);
                if (HI_SUCCESS != s32Ret )
                {
            ...
            s32Ret = HI_MPI_AI_ReleaseFrame(pstAiCtl->AiDev, pstAiCtl->AiChn, &stFrame, &stAecFrame);
            if (HI_SUCCESS != s32Ret )
            {
...
void *SAMPLE_COMM_AUDIO_AdecProc(void *parg)
{
    ...
    while (HI_TRUE == pstAdecCtl->bStart)
    {
        ...
        {                        
            ...
            #if 0
            stAudioStream.pStream = pu8AudioStream;
            u32ReadLen = fread(stAudioStream.pStream, 1, 151, pfd); 
            stAudioStream.u32Len = u32ReadLen;
            s32Ret = HI_MPI_ADEC_SendStream(s32AdecChn, &stAudioStream, HI_TRUE);           
            printf("=============\n");
            
            HI_MPI_ADEC_SendStreamEOF(s32AdecChn, HI_FALSE);
            fseek(pfd, 0, SEEK_SET);/*read file again*/  
            sleep(20);
            printf("~~~~~~~~~~~~~\n");
            #endif
        ...
        AIO_ATTR_S *pstAioAttr, HI_BOOL bAnrEn, AUDIO_RESAMPLE_ATTR_S *pstAiReSmpAttr)
{
    ...
    for (i=0; i<s32AiChnCnt; i++)
    {
        ...
        if (HI_TRUE == bAnrEn)
        {
            if(HI_MPI_AI_GetVqeAttr(AiDevId, i, &AiVqeCfg))
            {
                printf("%s: HI_MPI_AI_GetVqeAttr failed with %#x\n", __FUNCTION__, s32Ret);
                return HI_FAILURE;
            }
            AiVqeCfg.bAnrOpen = HI_TRUE;
            if (HI_MPI_AI_SetVqeAttr(AiDevId, i, 0, 0, &AiVqeCfg))
            {
                printf("%s: HI_MHI_MPI_AI_SetVqeAttr failed with %#x\n", __FUNCTION__, s32Ret);
                return HI_FAILURE;
            }
            if (HI_MPI_AI_EnableVqe(AiDevId, i))
            {
                printf("%s: HI_MHI_MPI_AI_SetVqeAttr failed with %#x\n", __FUNCTION__, s32Ret);
                return HI_FAILURE;
            }
        ...
        {
            if (HI_MPI_AI_EnableReSmp(AiDevId, i, pstAiReSmpAttr->enOutSampleRate))
            {
        ...
        AIO_ATTR_S *pstAioAttr, AUDIO_RESAMPLE_ATTR_S *pstAoReSmpAttr)
{
    ...
    if (NULL != pstAoReSmpAttr)
    {
        ...
        s32Ret |= HI_MPI_AO_EnableReSmp(AoDevId, AoChn, pstAoReSmpAttr->enInSampleRate);
        if(HI_SUCCESS != s32Ret)
        {
