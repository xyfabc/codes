...
//#define ZMJ_LABEL_HIGHT 100   //add by eric.xi@zmj at 20191121
HI_S32 SAMPLE_COMM_VO_StartChn(VO_LAYER VoLayer, SAMPLE_VO_MODE_E enMode)
{
    ...
    u32Height = stLayerAttr.stImageSize.u32Height-ZMJ_LOGO_BAR_HIGHT;
    printf("u32Width:%d, u32Square:%d\n", u32Width, u32Square);
    for (i=0; i<u32WndNum; i++)
    {
        stChnAttr.stRect.s32X       = ALIGN_BACK((u32Width/u32Square) * (i%u32Square), 2);
        stChnAttr.stRect.s32Y       = ZMJ_LOGO_BAR_HIGHT+ALIGN_BACK((u32Height/u32Square) * (i/u32Square), 2);
        ...
        printf("%s(%d):------%d:%d:%d:%d!\n",__FUNCTION__,__LINE__,  stChnAttr.stRect.s32X,stChnAttr.stRect.s32Y,stChnAttr.stRect.u32Width,stChnAttr.stRect.u32Height);
...
HI_S32 ZMJ_COMM_VO_StartChn(VO_LAYER VoLayer, SAMPLE_VO_MODE_E enMode)
{
    HI_S32 i;
    HI_S32 s32Ret = HI_SUCCESS;
    HI_U32 u32WndNum = 0;
    ...
    if(VO_MODE_6MUX == enMode){

        for (i=0; i<u32WndNum; i++)
        {
            if(i == 0){
                stChnAttr.stRect.s32X       = ALIGN_BACK((u32Width/u32Square) * (0%u32Square), 2);
                stChnAttr.stRect.s32Y       = ZMJ_LOGO_BAR_HIGHT+ALIGN_BACK((u32Height/u32Square) * (0/u32Square), 2);
                stChnAttr.stRect.u32Width   = ALIGN_BACK(2*u32Width/u32Square, 2);
                stChnAttr.stRect.u32Height  = ALIGN_BACK(2*u32Height/u32Square, 2);
            }else if(i == 1){
                stChnAttr.stRect.s32X       = ALIGN_BACK((u32Width/u32Square) * ((i+1)%u32Square), 2);
                stChnAttr.stRect.s32Y       = ZMJ_LOGO_BAR_HIGHT+ALIGN_BACK((u32Height/u32Square) * ((i+1)/u32Square), 2);
                stChnAttr.stRect.u32Width   = ALIGN_BACK(u32Width/u32Square, 2);
                stChnAttr.stRect.u32Height  = ALIGN_BACK(u32Height/u32Square, 2);
            }else{
                stChnAttr.stRect.s32X       = ALIGN_BACK((u32Width/u32Square) * ((i+3)%u32Square), 2);
                stChnAttr.stRect.s32Y       = ZMJ_LOGO_BAR_HIGHT+ALIGN_BACK((u32Height/u32Square) * ((i+3)/u32Square), 2);
                stChnAttr.stRect.u32Width   = ALIGN_BACK(u32Width/u32Square, 2);
                stChnAttr.stRect.u32Height  = ALIGN_BACK(u32Height/u32Square, 2);
            }
...
HI_S32 SAMPLE_COMM_VO_HdmiScreenStartup()
{
//    HI_HDMI_ATTR_S      stAttr;
...
HI_S32 SAMPLE_COMM_VO_HDMIChangeFormatStart(VO_INTF_SYNC_E enIntfSync)
{
    ...
    stAttr.bEnableHdmi = HI_TRUE;
    stAttr.bEnableVideo = HI_TRUE;
    SAMPLE_COMM_VO_HdmiConvertSync(enIntfSync, &stAttr.enVideoFmt);
    HI_MPI_HDMI_SetAttr(HI_HDMI_ID_0, &stAttr);
    HI_MPI_HDMI_Start(HI_HDMI_ID_0);
...
HI_S32 SAMPLE_COMM_VO_HDMIChangeFormatStop()
{
    HI_MPI_HDMI_Stop(HI_HDMI_ID_0);
