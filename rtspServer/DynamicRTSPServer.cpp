...
void DynamicRTSPServer::SetWorkThread(WorkThread *pwork)
{
    pWorkThread = pwork;
}

void DynamicRTSPServer::AddTable(table *tmptable)
{
    pWorkThread->addTable(tmptable);
}
...
static ServerMediaSession* createNewSMS(UsageEnvironment& env,
                                        char const* fileName, FILE* fid, DynamicRTSPServer *pDynamicRTSPServer); // forward
...
static ServerMediaSession* createNewSMS(UsageEnvironment& env,
                                        char const* fileName, FILE* /*fid*/,DynamicRTSPServer *pDynamicRTSPServer) {
    ...
    if (strcmp(extension, ".aac") == 0) {
    ...
    }else if (strcmp(extension, ".zmj") == 0) {
        // Assumed to be a H.265 Video Elementary Stream file:
        NEW_SMS("H.zmj Video");
        printf("--func:%s----------------line:%d--\n",__func__,__LINE__);
        ...
        OutPacketBuffer::maxSize = 1000000; // allow for some possibly large H.265 frames
        //pWorkThread;
        pDynamicRTSPServer->AddTable(table1);
        printf("--func:%s----------------line:%d--\n",__func__,__LINE__);
