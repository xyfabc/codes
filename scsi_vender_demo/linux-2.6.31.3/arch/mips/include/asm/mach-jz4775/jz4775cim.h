/*
 * linux/include/asm-mips/mach-jz4775/jz4775cim.h
 *
 * JZ4775 CIM register definition.
 *
...
#define	CIM0_BASE	0xB3060000
#define	CIM1_BASE	0xB30E0000

/*************************************************************************
 * CIM
 *************************************************************************/
#define	CIM_CFG(n)		(CIM0_BASE + (n)*0x80000 + 0x0000)
#define	CIM_CTRL(n)		(CIM0_BASE + (n)*0x80000 + 0x0004)
#define	CIM_STATE(n)		(CIM0_BASE + (n)*0x80000 + 0x0008)
#define	CIM_IID(n)		(CIM0_BASE + (n)*0x80000 + 0x000C)
#define	CIM_DA(n)		(CIM0_BASE + (n)*0x80000 + 0x0020)
#define	CIM_FA(n)		(CIM0_BASE + (n)*0x80000 + 0x0024)
#define	CIM_FID(n)		(CIM0_BASE + (n)*0x80000 + 0x0028)
#define	CIM_CMD(n)		(CIM0_BASE + (n)*0x80000 + 0x002C)
#define	CIM_SIZE(n)		(CIM0_BASE + (n)*0x80000 + 0x0030)
#define	CIM_OFFSET(n)		(CIM0_BASE + (n)*0x80000 + 0x0034)
#define CIM_YFA(n)		(CIM0_BASE + (n)*0x80000 + 0x0038)
#define CIM_YCMD(n)		(CIM0_BASE + (n)*0x80000 + 0x003C)
#define CIM_CBFA(n)		(CIM0_BASE + (n)*0x80000 + 0x0040)
#define CIM_CBCMD(n)		(CIM0_BASE + (n)*0x80000 + 0x0044)
#define CIM_CRFA(n)		(CIM0_BASE + (n)*0x80000 + 0x0048)
#define CIM_CRCMD(n)		(CIM0_BASE + (n)*0x80000 + 0x004C)
#define CIM_CTRL2(n)		(CIM0_BASE + (n)*0x80000 + 0x0050)
#define CIM_FS(n)		(CIM0_BASE + (n)*0x80000 + 0x0054)
#define CIM_IMR(n)		(CIM0_BASE + (n)*0x80000 + 0x0058)
#define CIM_TC(n)		(CIM0_BASE + (n)*0x80000 + 0x005C)
#define CIM_TINX(n)		(CIM0_BASE + (n)*0x80000 + 0x0060)
#define CIM_TCNT(n)		(CIM0_BASE + (n)*0x80000 + 0x0064)
...
#define __cim_sw_reset(n)	( REG_CIM_CTRL(n) |= CIM_CTRL_SWRST )

#define __cim_enable_sep(n)	 (REG_CIM_CFG(n) |= CIM_CFG_SEP)
#define __cim_disable_sep(n)	 (REG_CIM_CFG(n) &= ~CIM_CFG_SEP)

/* n = 0, 1, 2, 3 */
#define __cim_set_input_data_stream_order(n, a)				\
	do {								\
		REG_CIM_CFG(n) &= ~CIM_CFG_ORDER_MASK;			\
		REG_CIM_CFG(n) |= ((a)<<CIM_CFG_ORDER_BIT)&CIM_CFG_ORDER_MASK; \
	} while (0)
...
/* n=0-7 */
#define __cim_set_data_packing_mode(n, a) 		\
do {							\
	REG_CIM_CFG(n) &= ~CIM_CFG_PACK_MASK;		\
	REG_CIM_CFG(n) |= ((a) << CIM_CFG_PACK_BIT);	\
} while (0)

//#define __cim_enable_bypass_func() 	(REG_CIM_CFG(n) |= CIM_CFG_BYPASS)
//#define __cim_disable_bypass_func() 	(REG_CIM_CFG(n) &= ~CIM_CFG_BYPASS_MASK)

#define __cim_enable_bypass_func(n) 	(REG_CIM_CTRL2(n) &= ~CIM_CTRL2_CSC_MASK)

#define __cim_enable_framesize_check(n)	( REG_CIM_CTRL2(n) |= CIM_CTRL2_FSC )
#define __cim_disable_framesize_check(n)	( REG_CIM_CTRL2(n) &= ~CIM_CTRL2_FSC )

#define __cim_enable_arif(n)	( REG_CIM_CTRL2(n) |= CIM_CTRL2_ARIF )
#define __cim_disable_arif(n)	( REG_CIM_CTRL2(n) &= ~CIM_CTRL2_ARIF )

#define __cim_ignore_h_frame_reminder(n)	( REG_CIM_CTRL2(n) |= CIM_CTRL2_FRAGHE )
#define __cim_ignore_v_frame_reminder(n)	( REG_CIM_CTRL2(n) |= CIM_CTRL2_FRAGVE )

#define __cim_csc_yuv422(n)		( REG_CIM_CTRL2(n) |= CIM_CTRL2_CSC_YUV422 )
#define __cim_csc_yuv420(n)		( REG_CIM_CTRL2(n) |= CIM_CTRL2_CSC_YUV420 )

#define __cim_set_h_down_scale(n, a)		\
do {						\
	REG_CIM_CTRL2(n) &= ~CIM_CTRL2_HRS_MASK;	\
	REG_CIM_CTRL2(n) |= (((a) & 0x3) << CIM_CTRL2_HRS_BIT);	\
} while(0)

#define __cim_set_v_down_scale(n, a)		\
do {						\
	REG_CIM_CTRL2(n) &= ~CIM_CTRL2_VRS_MASK;	\
	REG_CIM_CTRL2(n) |= (((a) & 0x3) << CIM_CTRL2_VRS_BIT);	\
} while(0)
...
#define __cim_enable_nongated_clock_mode(n)	\
do {						\
	REG_CIM_CFG(n) &= ~CIM_CFG_DSM_MASK;	\
	REG_CIM_CFG(n) |= CIM_CFG_DSM_NGCM;	\
} while (0)

#define __cim_enable_eeof(n) \
	( REG_CIM_CFG(n) |= CIM_CFG_EEOFEN )

/* n=1-16 */
#define __cim_set_frame_rate(n, a) 		\
do {						\
	REG_CIM_CTRL(n) &= ~CIM_CTRL_FRC_MASK; 	\
	REG_CIM_CTRL(n) |= CIM_CTRL_FRC_##a;	\
} while (0)
...
#define __cim_enable_mb(n) \
	( REG_CIM_CTRL(n) |= CIM_CTRL_MBEN)
#define __cim_disable_mb(n) \
	( REG_CIM_CTRL(n) &= ~CIM_CTRL_MBEN )

#define __cim_enable_vdd_intr(n) \
	( REG_CIM_IMR(n) &= ~CIM_IMR_VDD_MASK )
#define __cim_disable_vdd_intr(n) \
	( REG_CIM_IMR(n) |= CIM_IMR_VDD_MASK )

#define __cim_enable_sof_intr(n) \
	( REG_CIM_IMR(n) &= ~CIM_IMR_DMA_SOF_MASK )
#define __cim_disable_sof_intr(n) \
	( REG_CIM_IMR(n) |= CIM_IMR_DMA_SOF_MASK )

#define __cim_enable_eof_intr(n) \
	( REG_CIM_IMR(n) &= ~CIM_IMR_DMA_EOF_MASK )
#define __cim_disable_eof_intr(n) \
	( REG_CIM_IMR(n) |= CIM_IMR_DMA_EOF_MASK )

#define __cim_enable_eeof_intr(n) \
	( REG_CIM_IMR(n) &= ~CIM_IMR_DMA_EEOF_MASK )
#define __cim_disable_eeof_intr(n) \
	( REG_CIM_IMR(n) |= CIM_IMR_DMA_EEOF_MASK )

#define __cim_enable_stop_intr(n) \
	( REG_CIM_IMR(n) &= ~CIM_IMR_DMA_STOP_MASK )
#define __cim_disable_stop_intr(n) \
	( REG_CIM_IMR(n) |= CIM_IMR_DMA_STOP_MASK )

#define __cim_enable_rxfifo_overflow_intr(n) \
	( REG_CIM_IMR(n) &= ~CIM_IMR_RXF_OF_MASK )
#define __cim_disable_rxfifo_overflow_intr(n) \
	( REG_CIM_IMR(n) |= CIM_CTRL_RXF_OF_MASK )

#define __cim_enable_tlb_error_intr(n) \
	( REG_CIM_IMR(n) &= ~CIM_IMR_TLBE_MASK )
#define __cim_disable_tlb_error_intr(n) \
	( REG_CIM_IMR(n) |= CIM_IMR_TLBE_MASK )

#define __cim_enable_framesize_check_error_intr(n) \
	( REG_CIM_IMR(n) &= ~CIM_IMR_FSE_MASK )
#define __cim_disable_framesize_check_error_intr(n) \
	( REG_CIM_IMR(n) |= CIM_IMR_FSE_MASK )

#define __cim_set_eeof_line(n, a)						\
	do {								\
		REG_CIM_CTRL(n) &= ~CIM_CTRL_EEOF_LINE_MASK;		\
		REG_CIM_CTRL(n) |= ( ((a) << CIM_CTRL_EEOF_LINE_BIT) & CIM_CTRL_EEOF_LINE_MASK ); \
	} while (0)
 ...
 */
#define __cim_set_opg(n, a)				\
	do {								\
		REG_CIM_CTRL2(n) &= ~CIM_CTRL2_OPG_MASK;			\
		REG_CIM_CTRL2(n) |= ((a) << CIM_CTRL2_OPG_BIT) & CIM_CTRL2_OPG_MASK; \
	} while (0)
...
#define __cim_dma_sof(n)		( REG_CIM_STATE(n) & CIM_STATE_DMA_SOF )
#define __cim_tlb_error(n)		( REG_CIM_STATE(n) & CIM_STATE_TLBE )
#define __cim_framesize_check_error(n)   	     ( REG_CIM_STATE(n) & CIM_STATE_FSE )

#define __cim_get_iid(n)		( REG_CIM_IID(n) )
#define __cim_get_fid(n)		( REG_CIM_FID(n) )
#define __cim_get_image_data(n)		( REG_CIM_RXFIFO(n) )
...
#define __cim_set_fs_v_size(n, a)	( REG_CIM_FS(n) = (REG_CIM_FS(n) &(~CIM_FS_FVS_MASK)) | ((a)<<CIM_FS_FVS_BIT) )
#define __cim_set_fs_h_size(n, a)	( REG_CIM_FS(n) = (REG_CIM_FS(n) &(~CIM_FS_FHS_MASK)) | ((a)<<CIM_FS_FHS_BIT) )
#define __cim_set_fs_bpp(n, a)		( REG_CIM_FS(n) = (REG_CIM_FS(n) &(~CIM_FS_BPP_MASK)) | ((a)<<CIM_FS_BPP_BIT) )

#define __cim_enable_tlb(n)	( REG_CIM_TC(n) |= CIM_TC_ENB )
#define __cim_disable_tlb(n)	( REG_CIM_TC(n) &= ~CIM_TC_ENB )
#define __cim_reset_tlb(n)	( REG_CIM_TC(n) |= CIM_TC_RST )
#define __cim_unreset_tlb(n)	( REG_CIM_TC(n) &= ~CIM_TC_RST )
#define __cim_set_tlb_addr(n, a)	( REG_CIM_TC(n) = (REG_CIM_TC(n) & (~CIM_TC_TBA_MASK)) | ((a) << CIM_TC_TBA_BIT) )

#define __cim_set_tlb_idx(n, a)	( REG_CIM_TINX(n) = (REG_CIM_TINX(n) & (~CIM_TINX_INDEX_MASK)) | ((a) << CIM_TINX_INDEX_BIT) )

#define __cim_set_tlb_cnt(n, a)	( REG_CIM_TCNT(n) = (a) )
