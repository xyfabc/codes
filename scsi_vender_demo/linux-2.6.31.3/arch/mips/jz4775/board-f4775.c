/*
 * linux/arch/mips/jz4775/board-f4775.c
 *
 * JZ4775 F4775 board setup routines.
 *
...
static void f4775_sd_gpio_init(struct device *dev)
{
...
static void f4775_sd_power_on(struct device *dev)
{
...
static void f4775_sd_power_off(struct device *dev)
{
#ifndef CONFIG_JZ_FPGA
...
static void f4775_sd_cpm_start(struct device *dev)
{
#ifndef CONFIG_JZ_FPGA
...
static unsigned int f4775_sd_status(struct device *dev)
{
...
static void f4775_sd_plug_change(int state)
{
...
static unsigned int f4775_sd_get_wp(struct device *dev)
{
#if defined(CONFIG_JZ_SYSTEM_AT_CARD) || defined(CONFIG_JZ_FPGA)
	return 0;
#else
...
#if defined(CONFIG_JZ_SYSTEM_AT_CARD)
struct mmc_partition_info f4775_partitions[] = {
	[0] = {"mbr",0,512,0},//0 - 512
...
#endif
struct jz_mmc_platform_data f4775_sd_data = {
...
#endif
	.init           = f4775_sd_gpio_init,
	.power_on       = f4775_sd_power_on,
	.power_off      = f4775_sd_power_off,
	.cpm_start	= f4775_sd_cpm_start,
	.status		= f4775_sd_status,
	.plug_change	= f4775_sd_plug_change,
	.write_protect  = f4775_sd_get_wp,
...
#endif
#if defined(CONFIG_JZ_SYSTEM_AT_CARD)
	.partitions = f4775_partitions,
	.num_partitions = ARRAY_SIZE(f4775_partitions),
#endif
};

static void f4775_tf_gpio_init(struct device *dev)
{
...
static void f4775_tf_power_on(struct device *dev)
{
#ifndef CONFIG_JZ_FPGA
...
static void f4775_tf_power_off(struct device *dev)
{
#ifndef CONFIG_JZ_FPGA
...
static void f4775_tf_cpm_start(struct device *dev)
{
#ifndef CONFIG_JZ_FPGA
...
static unsigned int f4775_tf_status(struct device *dev)
{
#if defined(CONFIG_JZ_SYSTEM_AT_CARD) || defined(CONFIG_JZ_FPGA)
	return 1;
#else
...
static void f4775_tf_plug_change(int state)
{
	if(state == CARD_INSERTED) /* wait for remove */
#if ACTIVE_LOW_MSC1_CD
...
struct jz_mmc_platform_data f4775_tf_data = {
...
#endif
	.init           = f4775_tf_gpio_init,
	.power_on       = f4775_tf_power_on,
	.power_off      = f4775_tf_power_off,
	.cpm_start	= f4775_tf_cpm_start,
	.status		= f4775_tf_status,
	.plug_change	= f4775_tf_plug_change,
...
static void f4775_msc2_gpio_init(struct device *dev)
{
	__gpio_as_msc2_4bit();
	return;
}

static void f4775_msc2_power_on(struct device *dev)
{
	return;
}

static void f4775_msc2_power_off(struct device *dev)
{
	return;
}

static void f4775_msc2_cpm_start(struct device *dev)
{
#ifndef CONFIG_JZ_FPGA
...
static unsigned int f4775_msc2_status(struct device *dev)
{
	return 1;	      /* default: no card */
}

static void f4775_msc2_plug_change(int state)
{
	return;
}

struct jz_mmc_platform_data f4775_msc2_data = {
	...
	.detect_pin     = GPIO_SD1_CD_N,
#endif
	.init           = f4775_msc2_gpio_init,
	.power_on       = f4775_msc2_power_on,
	.power_off      = f4775_msc2_power_off,
	.cpm_start	= f4775_msc2_cpm_start,
	.status		= f4775_msc2_status,
	.plug_change	= f4775_msc2_plug_change,
	.max_bus_width  = MMC_CAP_SD_HIGHSPEED
...
{
	jz_add_msc_devices(0, &f4775_sd_data);
	jz_add_msc_devices(1, &f4775_tf_data);
	jz_add_msc_devices(2, &f4775_msc2_data);
}
...
void __init jz_board_setup(void)
{
	...
	printk("JZ4775 F4775 board setup\n");
//	jz_restart(NULL);
	board_cpm_setup();
	board_gpio_setup();
}
