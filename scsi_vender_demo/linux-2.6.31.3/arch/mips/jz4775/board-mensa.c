/*
 * linux/arch/mips/jz4775/board-mensa.c
 *
...
static void mensa_inand_gpio_init(struct device *dev)
{
...
static void mensa_inand_power_on(struct device *dev)
{
}

static void mensa_inand_power_off(struct device *dev)
{
}

static void mensa_inand_cpm_start(struct device *dev)
{
...
static unsigned int mensa_inand_status(struct device *dev)
{
	return 1;
}

static void mensa_inand_plug_change(int state)
{
	return ;
}

static unsigned int mensa_inand_get_wp(struct device *dev)
{
	return 0;
}

#if defined(CONFIG_JZ_SYSTEM_AT_CARD)
struct mmc_partition_info mensa_partitions[] = {
	[0] = {"mbr",0,512,0},//0 - 512
...
#endif
struct jz_mmc_platform_data mensa_inand_data = {
	...
	.status_irq	= 0,
	.detect_pin     = 0,
	.init           = mensa_inand_gpio_init,
	.power_on	= mensa_inand_power_on,
	.power_off	= mensa_inand_power_off,
	.cpm_start	= mensa_inand_cpm_start,
	.status		= mensa_inand_status,
	.plug_change	= mensa_inand_plug_change,
	.write_protect	= mensa_inand_get_wp,
...
#endif
#if defined(CONFIG_JZ_SYSTEM_AT_CARD)
	.partitions = mensa_partitions,
	.num_partitions = ARRAY_SIZE(mensa_partitions),
#endif
};

static void mensa_sd_gpio_init(struct device *dev)
{
...
static void mensa_sd_power_on(struct device *dev)
{
#ifndef CONFIG_JZ_FPGA
...
static void mensa_sd_power_off(struct device *dev)
{
#ifndef CONFIG_JZ_FPGA
...
static void mensa_sd_cpm_start(struct device *dev)
{
#ifndef CONFIG_JZ_FPGA
...
static unsigned int mensa_sd_status(struct device *dev)
{
...
static void mensa_sd_plug_change(int state)
{
...
struct jz_mmc_platform_data mensa_sd_data = {
...
#endif
	.init           = mensa_sd_gpio_init,
	.power_on	= mensa_sd_power_on,
	.power_off	= mensa_sd_power_off,
	.cpm_start	= mensa_sd_cpm_start,
	.status		= mensa_sd_status,
	.plug_change	= mensa_sd_plug_change,
...
static void mensa_tf_gpio_init(struct device *dev)
{
...
static void mensa_tf_power_on(struct device *dev)
{
#ifndef CONFIG_JZ_FPGA
	__gpio_as_output1(GPIO_SD1_VCC_EN_N);
#endif
...
static void mensa_tf_power_off(struct device *dev)
{
#ifndef CONFIG_JZ_FPGA
	__gpio_as_output0(GPIO_SD1_VCC_EN_N);
#endif
...
static void mensa_tf_cpm_start(struct device *dev)
{
#ifndef CONFIG_JZ_FPGA
...
static unsigned int mensa_tf_status(struct device *dev)
{
	unsigned int status = 0;
...
static void mensa_tf_plug_change(int state)
{
	if(state == CARD_INSERTED) /* wait for remove */
#if ACTIVE_LOW_MSC1_CD
//#if ACTIVE_LOW_MSC0_CD
...
struct jz_mmc_platform_data mensa_tf_data = {
...
//#if defined(CONFIG_JZ_SYSTEM_AT_CARD) || defined(CONFIG_JZ_FPGA)
	...
	.detect_pin     = GPIO_SD1_CD_N,
#endif
	.init           = mensa_tf_gpio_init,
	.power_on	= mensa_tf_power_on,
	.power_off	= mensa_tf_power_off,
	.cpm_start	= mensa_tf_cpm_start,
	.status		= mensa_tf_status,
	.plug_change	= mensa_tf_plug_change,
	.max_bus_width  = MMC_CAP_SD_HIGHSPEED
...
{
	jz_add_msc_devices(0, &mensa_inand_data);
	jz_add_msc_devices(1, &mensa_sd_data);
	jz_add_msc_devices(2, &mensa_tf_data);
}
...
static struct act8600_outputs_t act8600_outputs[] = {
	...
	{4,0x5b,0},//out4 5.7v for otg connect wifi
	//{5,0x31,1},//out5 2.5  - 0b110001
	//{6,0x39,0},//out6 LCD3.3V  - 0b111001
	{7,0x39,0},//out7 1.2V CON - 0b011000
...
static struct i2c_board_info f4775_i2c0_devs[] __initdata = {
	...
	/*{ 
		I2C_BOARD_INFO("hym8563", (0xA2 >> 1)) 
	},*/
	{
		I2C_BOARD_INFO("spls", 0x60) 
	},
#if 0
	{
		I2C_BOARD_INFO("jz7410", 0x28),
...
static struct i2c_board_info f4775_i2c2_devs[] __initdata = {
	...
	{	I2C_BOARD_INFO("gc0308_yuv", 0x21),	},
	/*{ 
		I2C_BOARD_INFO("ov7725_yuv", 0x21) 
	},*/
	//{ I2C_BOARD_INFO("ov7725_rgb",0X20) }, 
...
void __init jz_board_setup(void)
{
	...
	printk("JZ4775 MENSA board setup\n");
//	jz_restart(NULL);
	board_cpm_setup();
	board_gpio_setup();
}
