/*
 * linux/arch/mips/jz4775/cpufreq.c
 *
 * cpufreq driver for JZ4775 
 *
...
struct jz4775_freq_percpu_info {
	struct cpufreq_frequency_table table[7];
};

static struct jz4775_freq_percpu_info jz4775_freq_table;
...
static void jz4775_transition(struct dpm_regs *regs)
{
	/*
...
static unsigned int jz4775_freq_get(unsigned int cpu)
{
...
static unsigned int index_to_divisor(unsigned int index, struct dpm_regs *regs)
{
	...
	new_freq = jz4775_freq_table.table[index].frequency;
...
static void jz4775_set_cpu_divider_index(unsigned int cpu, unsigned int index)
{
	...
	if (old_divisor != divisor)
		jz4775_transition(&regs);

	cpufreq_notify_transition(&freqs, CPUFREQ_POSTCHANGE);
}

static int jz4775_freq_target(struct cpufreq_policy *policy,
			  ...
			  unsigned int relation)
{
	...
	if (cpufreq_frequency_table_target(policy,
					   &jz4775_freq_table.table[0],
					   target_freq, relation, &new_index))
		return -EINVAL;

	jz4775_set_cpu_divider_index(policy->cpu, new_index);
...
static int jz4775_freq_verify(struct cpufreq_policy *policy)
{
	return cpufreq_frequency_table_verify(policy,
					      &jz4775_freq_table.table[0]);
}

static int __init jz4775_cpufreq_driver_init(struct cpufreq_policy *policy)
{

	struct cpufreq_frequency_table *table =	&jz4775_freq_table.table[0];
	unsigned int MAX_FREQ;

	dprintk(KERN_INFO "Jz4775 cpufreq driver\n");
...
static struct cpufreq_driver cpufreq_jz4775_driver = {
//	.flags		= CPUFREQ_STICKY,
	.init		= jz4775_cpufreq_driver_init,
	.verify		= jz4775_freq_verify,
	.target		= jz4775_freq_target,
	.get		= jz4775_freq_get,
	.name		= "jz4775",
};

static int __init jz4775_cpufreq_init(void)
{
	return cpufreq_register_driver(&cpufreq_jz4775_driver);
}

static void __exit jz4775_cpufreq_exit(void)
{
	cpufreq_unregister_driver(&cpufreq_jz4775_driver);
}

module_init(jz4775_cpufreq_init);
module_exit(jz4775_cpufreq_exit);

MODULE_AUTHOR("Regen <lhhuang@ingenic.cn>");
MODULE_DESCRIPTION("cpufreq driver for Jz4775");
MODULE_LICENSE("GPL");
