/*
 * linux/arch/mips/jz4775/common/setup.c
 *
 * JZ4775 common setup routines.
 *
...
// extern void jz_pm_hibernate(void);	// del, yjt, 20140106

static void __init sysclocks_setup(void)
{
#ifndef CONFIG_FPGA 
	jz_clocks.cclk = cpm_get_clock(CGU_CCLK);
	jz_clocks.l2clk = cpm_get_clock(CGU_L2CLK);
	...
	jz_clocks.vpuclk = cpm_get_clock(CGU_VPUCLK);
...
#define FPGACLK 24000000

	jz_clocks.cclk = jz_clocks.l2clk = FPGACLK * 2;
	jz_clocks.hclk = FPGACLK;
	jz_clocks.h2clk = FPGACLK;
	jz_clocks.pclk = FPGACLK;
	jz_clocks.mclk = FPGACLK;
	jz_clocks.pixclk0 = FPGACLK;
	jz_clocks.pixclk1 = FPGACLK;
	jz_clocks.i2sclk = FPGACLK;
	jz_clocks.otgclk = FPGACLK;
	jz_clocks.msc0clk = FPGACLK;
	jz_clocks.msc1clk = FPGACLK;
	jz_clocks.msc2clk = FPGACLK;
	jz_clocks.extalclk = FPGACLK;
	jz_clocks.rtcclk = FPGACLK;
#endif

	printk("CPU clock: %dMHz, System clock: %dMHz, Peripheral clock: %dMHz, Memory clock: %dMHz, AHB2 clock: %dMHz\n",
...
static void __init jz_soc_setup(void)
{
	...
	soc_harb_setup();
	soc_dmac_setup();
	REG32(0xb0000048) &= ~(1 << 17);	//fix:twxie. default suspend ohci PHY
}
...
void __init plat_mem_setup(void)
{
	...
	// pm_power_off = jz_pm_hibernate;	// del, yjt, 20140106
