/*
 * linux/arch/mips/jz4780/board-f4780.c
 *
 * JZ4780 F4780 board setup routines.
 *
...
static void f4780_sd_gpio_init(struct device *dev)
{
...
static void f4780_sd_power_on(struct device *dev)
{
	__gpio_as_output0(GPIO_SD0_VCC_EN_N);
}

static void f4780_sd_power_off(struct device *dev)
{
	__gpio_as_output1(GPIO_SD0_VCC_EN_N);
}

static void f4780_sd_cpm_start(struct device *dev)
{
	printk("===>start MSC0 clock\n");
	cpm_start_clock(CGM_MSC0);
}

static unsigned int f4780_sd_status(struct device *dev)
{
#if defined(CONFIG_JZ_SYSTEM_AT_CARD)
	return 1;
#else
...
static void f4780_sd_plug_change(int state)
{
...
static unsigned int f4780_sd_get_wp(struct device *dev)
{
#if defined(CONFIG_JZ_SYSTEM_AT_CARD)
	return 0;
#else
...
#if defined(CONFIG_JZ_SYSTEM_AT_CARD)
struct mmc_partition_info f4780_partitions[] = {
	[0] = {"mbr",0,512,0},//0 - 512
...
#endif
struct jz_mmc_platform_data f4780_sd_data = {
...
#endif
	.init           = f4780_sd_gpio_init,
	.power_on       = f4780_sd_power_on,
	.power_off      = f4780_sd_power_off,
	.cpm_start	= f4780_sd_cpm_start,
	.status		= f4780_sd_status,
	.plug_change	= f4780_sd_plug_change,
	.write_protect  = f4780_sd_get_wp,
...
#endif
#if defined(CONFIG_JZ_SYSTEM_AT_CARD)
	.partitions = f4780_partitions,
	.num_partitions = ARRAY_SIZE(f4780_partitions),
#endif
};

static void f4780_tf_gpio_init(struct device *dev)
{
	__gpio_as_msc1_4bit();
//	__gpio_as_output1(GPIO_SD1_VCC_EN_N); /* poweroff */
//	__gpio_as_input(GPIO_SD1_CD_N);
		printk("REG_GPIO_PXINTC(4):%x\n",REG_GPIO_PXINT(4));
		printk("REG_GPIO_PXMASKC(4):%x\n",REG_GPIO_PXMASK(4));
		printk("REG_GPIO_PXPAT1C(4):%x\n",REG_GPIO_PXPAT1(4));
		printk("REG_GPIO_PXPAT0S(4):%x\n",REG_GPIO_PXPAT0(4));
}

static void f4780_tf_power_on(struct device *dev)
{
//	__gpio_as_output0(GPIO_SD1_VCC_EN_N);
}

static void f4780_tf_power_off(struct device *dev)
{
//	__gpio_as_output1(GPIO_SD1_VCC_EN_N);
}

static void f4780_tf_cpm_start(struct device *dev)
{
	cpm_start_clock(CGM_MSC1);
}

static unsigned int f4780_tf_status(struct device *dev)
{
#if defined(CONFIG_JZ_SYSTEM_AT_CARD)
	return 1;
#else
...
static void f4780_tf_plug_change(int state)
{
	if(state == CARD_INSERTED) /* wait for remove */
#if ACTIVE_LOW_MSC1_CD
...
struct jz_mmc_platform_data f4780_tf_data = {
...
#endif
	.init           = f4780_tf_gpio_init,
	.power_on       = f4780_tf_power_on,
	.power_off      = f4780_tf_power_off,
	.cpm_start	= f4780_tf_cpm_start,
	.status		= f4780_tf_status,
	.plug_change	= f4780_tf_plug_change,
...
static void f4780_msc2_gpio_init(struct device *dev)
{
	__gpio_as_msc2_4bit();
	return;
}

static void f4780_msc2_power_on(struct device *dev)
{
	return;
}

static void f4780_msc2_power_off(struct device *dev)
{
	return;
}

static void f4780_msc2_cpm_start(struct device *dev)
{
	cpm_start_clock(CGM_MSC2);
}

static unsigned int f4780_msc2_status(struct device *dev)
{
	return 1;	      /* default: no card */
}

static void f4780_msc2_plug_change(int state)
{
	return;
}

struct jz_mmc_platform_data f4780_msc2_data = {
	...
	.init           = f4780_msc2_gpio_init,
	.power_on       = f4780_msc2_power_on,
	.power_off      = f4780_msc2_power_off,
	.cpm_start	= f4780_msc2_cpm_start,
	.status		= f4780_msc2_status,
	.plug_change	= f4780_msc2_plug_change,
	.max_bus_width  = MMC_CAP_SD_HIGHSPEED
...
{
	jz_add_msc_devices(0, &f4780_sd_data);
	jz_add_msc_devices(1, &f4780_tf_data);
	jz_add_msc_devices(2, &f4780_msc2_data);
}
...
void __init jz_board_setup(void)
{
	...
	printk("JZ4780 F4780 board setup\n");
//	jz_restart(NULL);
	board_cpm_setup();
	board_gpio_setup();
}
