/*
 * linux/arch/mips/jz4780/board-norma.c
 *
 * JZ4780 NORMA board setup routines.
 *
...
static void norma_sd_gpio_init(struct device *dev)
{
...
static void norma_sd_power_on(struct device *dev)
{
	__gpio_as_output0(GPIO_SD0_VCC_EN_N);
}

static void norma_sd_power_off(struct device *dev)
{
	__gpio_as_output1(GPIO_SD0_VCC_EN_N);
}

static void norma_sd_cpm_start(struct device *dev)
{
	cpm_start_clock(CGM_MSC0);
}

static unsigned int norma_sd_status(struct device *dev)
{
#if defined(CONFIG_JZ_SYSTEM_AT_CARD)
	return 1;
#else
...
static void norma_sd_plug_change(int state)
{
...
static unsigned int norma_sd_get_wp(struct device *dev)
{
#if defined(CONFIG_JZ_SYSTEM_AT_CARD)
	return 0;
#else
...
#if defined(CONFIG_JZ_SYSTEM_AT_CARD)
struct mmc_partition_info norma_partitions[] = {
	[0] = {"mbr",0,512,0},//0 - 512
...
#endif
struct jz_mmc_platform_data norma_sd_data = {
...
#endif
	.init           = norma_sd_gpio_init,
	.power_on       = norma_sd_power_on,
	.power_off      = norma_sd_power_off,
	.cpm_start	= norma_sd_cpm_start,
	.status		= norma_sd_status,
	.plug_change	= norma_sd_plug_change,
	.write_protect  = norma_sd_get_wp,
...
#endif
#if defined(CONFIG_JZ_SYSTEM_AT_CARD)
	.partitions = norma_partitions,
	.num_partitions = ARRAY_SIZE(norma_partitions),
#endif
};

static void norma_tf_gpio_init(struct device *dev)
{
	__gpio_as_msc1_4bit();
...
static void norma_tf_power_on(struct device *dev)
{
	__gpio_as_output0(GPIO_SD1_VCC_EN_N);
}

static void norma_tf_power_off(struct device *dev)
{
	__gpio_as_output1(GPIO_SD1_VCC_EN_N);
}

static void norma_tf_cpm_start(struct device *dev)
{
	cpm_start_clock(CGM_MSC1);
}

static unsigned int norma_tf_status(struct device *dev)
{
...
static void norma_tf_plug_change(int state)
{
	if(state == CARD_INSERTED) /* wait for remove */
#if ACTIVE_LOW_MSC1_CD
...
struct jz_mmc_platform_data norma_tf_data = {
	...
	.init           = norma_tf_gpio_init,
	.power_on       = norma_tf_power_on,
	.power_off      = norma_tf_power_off,
	.cpm_start	= norma_tf_cpm_start,
	.status		= norma_tf_status,
	.plug_change	= norma_tf_plug_change,
...
static void norma_msc2_gpio_init(struct device *dev)
{
...
static void norma_msc2_power_on(struct device *dev)
{
	__gpio_as_output0(GPIO_SD0_VCC_EN_N);
}

static void norma_msc2_power_off(struct device *dev)
{
	__gpio_as_output1(GPIO_SD0_VCC_EN_N);
}

static void norma_msc2_cpm_start(struct device *dev)
{
	cpm_start_clock(CGM_MSC2);
}

static unsigned int norma_msc2_status(struct device *dev)
{
	unsigned int status = 0;
	status = (unsigned int) __gpio_get_pin(GPIO_SD0_CD_N);
#if ACTIVE_LOW_MSC0_CD
...
static void norma_msc2_plug_change(int state)
{
	if(state == CARD_INSERTED) /* wait for remove */
#if ACTIVE_LOW_MSC0_CD
		__gpio_as_irq_high_level(MSC0_HOTPLUG_PIN);
...
struct jz_mmc_platform_data norma_msc2_data = {
	...
	.status_irq	= MSC0_HOTPLUG_IRQ,
	.detect_pin     = GPIO_SD0_CD_N,
	.init           = norma_msc2_gpio_init,
	.power_on       = norma_msc2_power_on,
	.power_off      = norma_msc2_power_off,
	.cpm_start	= norma_msc2_cpm_start,
	.status		= norma_msc2_status,
	.plug_change	= norma_msc2_plug_change,
	.max_bus_width  = MMC_CAP_SD_HIGHSPEED
...
{
	jz_add_msc_devices(0, &norma_sd_data);
	jz_add_msc_devices(1, &norma_tf_data);
	jz_add_msc_devices(2, &norma_msc2_data);
}
...
void __init jz_board_setup(void)
{
	...
	printk("JZ4780 NORMA board setup\n");
//	jz_restart(NULL);
	board_cpm_setup();
	board_gpio_setup();
}
