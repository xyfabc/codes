/*
 * linux/arch/mips/jz4780/cpufreq.c
 *
 * cpufreq driver for JZ4780 
 *
...
struct jz4780_freq_percpu_info {
	struct cpufreq_frequency_table table[7];
};

static struct jz4780_freq_percpu_info jz4780_freq_table;
...
static void jz4780_transition(struct dpm_regs *regs)
{
	/*
...
static unsigned int jz4780_freq_get(unsigned int cpu)
{
...
static unsigned int index_to_divisor(unsigned int index, struct dpm_regs *regs)
{
	...
	new_freq = jz4780_freq_table.table[index].frequency;
...
static void jz4780_set_cpu_divider_index(unsigned int cpu, unsigned int index)
{
	...
	if (old_divisor != divisor)
		jz4780_transition(&regs);

	cpufreq_notify_transition(&freqs, CPUFREQ_POSTCHANGE);
}

static int jz4780_freq_target(struct cpufreq_policy *policy,
			  ...
			  unsigned int relation)
{
	...
	if (cpufreq_frequency_table_target(policy,
					   &jz4780_freq_table.table[0],
					   target_freq, relation, &new_index))
		return -EINVAL;

	jz4780_set_cpu_divider_index(policy->cpu, new_index);
...
static int jz4780_freq_verify(struct cpufreq_policy *policy)
{
	return cpufreq_frequency_table_verify(policy,
					      &jz4780_freq_table.table[0]);
}

static int __init jz4780_cpufreq_driver_init(struct cpufreq_policy *policy)
{

	struct cpufreq_frequency_table *table =	&jz4780_freq_table.table[0];
	unsigned int MAX_FREQ;

	dprintk(KERN_INFO "Jz4780 cpufreq driver\n");
...
static struct cpufreq_driver cpufreq_jz4780_driver = {
//	.flags		= CPUFREQ_STICKY,
	.init		= jz4780_cpufreq_driver_init,
	.verify		= jz4780_freq_verify,
	.target		= jz4780_freq_target,
	.get		= jz4780_freq_get,
	.name		= "jz4780",
};
...
{
	return cpufreq_register_driver(&cpufreq_jz4780_driver);
}

static void __exit jz4780_cpufreq_exit(void)
{
	cpufreq_unregister_driver(&cpufreq_jz4780_driver);
}
...
MODULE_DESCRIPTION("cpufreq driver for Jz4780");
MODULE_LICENSE("GPL");
