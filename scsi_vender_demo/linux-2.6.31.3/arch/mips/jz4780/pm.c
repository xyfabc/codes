...
#if defined(CONFIG_RTC_DRV_JZ4780) && defined(CONFIG_BATTERY_JZ)
...
int jz_pm_do_hibernate(void)
{
	...
	while (1) {
		printk("We should NOT come here, please check the jz4780rtc.h!!!\n");
	};
#endif
...
//	*(volatile unsigned *)0xb30100bc = 0;
	...
	while ((REG_SADC_ENA & SADC_ENA_POWER) != SADC_ENA_POWER) {
        ...
        /* stop otg and uhc */
	CLRREG32(CPM_OPCR, OPCR_USBPHY_ENABLE);
...
#if defined(CONFIG_RTC_DRV_JZ4780)
	...
	/* like jz4780, for cpu 533M 1:2:4 */
	...
	OUTREG32(CPM_SLPC, ((unsigned)jz_cpu_resume | 0x20000000));
#endif
	...
	printk("LCR = 0x%08x\n", *(volatile unsigned *)0xb0000004);
	printk("OPCR = 0x%08x\n", *(volatile unsigned *)0xb0000024);
	printk("RSR = 0x%08x\n", *(volatile unsigned *)0xb0000008);
	printk("SLBC = 0x%08x\n", *(volatile unsigned *)0xb00000c8);
	printk("SLPC = 0x%08x\n", *(volatile unsigned *)0xb00000cc);
	asm volatile (
		".set mips32\n\t"
		"mfc0 %0, $14, 0\n\t"
		"ssnop\n\t"
...
void jz_flush_cache_all(void)
{
	...
	for (addr = K0BASE; addr < (K0BASE + 0x8000); addr += 32) {
		asm volatile (
			".set mips3\n\t"
			" cache %0, 0(%1)\n\t"
			".set mips2\n\t"
...
static int jz4780_pm_valid(suspend_state_t state)
{
