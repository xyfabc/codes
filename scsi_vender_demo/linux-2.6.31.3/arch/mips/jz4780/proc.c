/*
 * linux/arch/mips/jz4780/proc.c
 *
 * /proc/jz/ procfs for jz4780 on-chip modules.
 *
...
extern void jz4780_fpu_init(unsigned int round);
...
{
	REG_CPM_CLKGR0 = simple_strtoul(buffer, 0, 16);
	return count;
}
...
static int fpu_write_proc(struct file *file, const char *buffer, unsigned long count, void *data)
{
	...
	jz4780_fpu_init((unsigned int)buffer);
	return count;
}
...
#if 1
	/*
	 * Reserve a 16MB memory for IPU on JZ4780.
	 */
