...
#include "jzchars.h"
#include "jz4780_tssi.h"



#define IOCTL_TSSI_ADD0         0xc
#define IOCTL_TSSI_SET_TRIGGER	0xd
#define TSSI_NAME "tssi"

#define TSSI_MINOR 204         /* MAJOR: 10, MINOR: 16 */

#define TSSI0_IRQ   IRQ_TSSI0
...
static void tssi_add_pid(int pid_num, int pid)
{
	...
	{ 
		addr = TSSI_PID0 + ( n * 4 );
		REG32( addr ) |= ( (pid & 0x1fff) << 16 );    
		REG_TSSI_PEN |= ( 1 << (16 + n) );
	}
	else           
	{
		addr = TSSI_PID0 + ( n * 4 );
		REG32( addr ) |= pid & 0x1fff;   
		REG_TSSI_PEN |= ( 1 << n  );
	}
}
...
static unsigned int tssi_get_offset(void)
{
	...
	wait_event_interruptible(tssi->wait, tssi->num != 0);
...
static int jz_open(struct inode * inode, struct file * filp)
{
	...
	__intc_mask_irq(TSSI0_IRQ);
	tssi_config_filting();
	__tssi_soft_reset();
	__tssi_clear_state();
	printk("<==== zli open test ===>\n");
	dump_tssi_regs();
	return 0;
}
...
static int jz_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		...
		case IOCTL_TSSI_ENABLE :
			...
			__tssi_clear_state();
// 			dump_tssi_regs();
			__intc_ack_irq(TSSI0_IRQ);
			__intc_unmask_irq(TSSI0_IRQ);
			tssi->num = 0;
			__tssi_enable();
		...
		case IOCTL_TSSI_GET_OFFSET:
			{
				...
				if(ret){
					printk("JZ TSSI: put_user failed, return\n");
					return -EFAULT;	 
				}
			}
	}
...
static int tssi_dma_desc_init(void)
{
	struct jz_tssi_t *tssi = jz_tssi_g;
...
static int __init jztssi_init_module(void)
{
	...
	tssi->offset = 0;
	tssi->num = 0;
	tssi->pid_num = 0; //for add pid filter
	...
	retval = request_irq(TSSI0_IRQ, tssi_interrupt, IRQF_DISABLED, TSSI_NAME, (void*)jz_tssi_g);
	if (retval) {
		printk("unable to get IRQ %d",TSSI0_IRQ);
		return retval;
	}
...
MODULE_AUTHOR("zli <zli@ingenic.cn>");
