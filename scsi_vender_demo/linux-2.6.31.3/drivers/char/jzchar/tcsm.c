...
static int tcsm_open(struct inode *inode, struct file *filp)
{
	...
	REG_CPM_CLKGR1 &= ~CLKGR1_VPU;
	REG_CPM_CLKGR1 &= ~CLKGR1_GPVLC;
	REG_CPM_VPUCDR = (1 << 30) | (1 << 29) | 1;
	while (REG_CPM_VPUCDR & (1 << 28)) ;
	
//cpm_start_clock(CGM_VPU);
#endif
#if defined(CONFIG_SOC_JZ4775)
	{
		unsigned div;
		unsigned vpu_clk = 200000000; //200M
		unsigned int pll_clk = cpm_get_xpllout(SCLK_MPLL);

		REG_CPM_CLKGR0 &= ~CLKGR0_VPU;
		if((pll_clk % vpu_clk) * 2 >= vpu_clk)
			div = (pll_clk / vpu_clk) + 1;
		else
			div = pll_clk / vpu_clk;
		printk("=====> VPUCDR DIV is %d\n", div);

		REG_CPM_VPUCDR = (1 << 31) | (1 << 29) | (div - 1);
		while (REG_CPM_VPUCDR & (1 << 28)) ;

		cpm_start_clock(CGM_X2D);
	}
#endif
