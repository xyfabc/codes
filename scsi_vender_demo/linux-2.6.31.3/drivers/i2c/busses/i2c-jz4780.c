...
static struct jz_i2c_dma_info rx_dma_info[6] = {
	...
	},
	{
		.chan = 7,
		.i2c_id = 3,
		.dma_id = DMA_ID_I2C3_RX,
		.dma_req = 0x2b,
		.name = "i2c3 read",
#ifdef CONFIG_JZ_I2C3_USE_DMA
		.use_dma = 1,
#else
		.use_dma = 0,
#endif
	},
	{
		.chan = 9,
		.i2c_id = 4,
		.dma_id = DMA_ID_I2C4_RX,
		.dma_req = 0x2d,
		.name = "i2c2 read",
#ifdef CONFIG_JZ_I2C4_USE_DMA
		.use_dma = 1,
#else
		.use_dma = 0,
#endif
...
#if defined(CONFIG_SOC_JZ4810) || defined(CONFIG_SOC_JZ4770) || defined (CONFIG_SOC_JZ4780)
	/*
		...
		int length, struct jz_i2c *i2c, int restart)
{
	...
	if (!restart)
		CLRREG8(I2C_CTRL(i2c_id),I2C_CTRL_STPHLD);
	printk("write address is 0x%02x   i = %d\n", wbuf[0], i);
...
static int i2c_jz_probe(struct platform_device *pdev)
{
...
#else
	if(rx_dma->use_dma){
		if (request_irq(IRQ_DMA_0 + tx_dma->chan, jz_i2c_dma_callback, IRQF_DISABLED, tx_dma->name, tx_dma)) {
			printk("===>chan %d: request irq failed!\n", tx_dma->chan);
		} else
			printk("===>chan %d: request irq succeed!\n", tx_dma->chan);

		if (request_irq(IRQ_DMA_0 + rx_dma->chan, jz_i2c_dma_callback, IRQF_DISABLED, rx_dma->name, rx_dma)) {
			printk("===>chan %d: request irq failed!\n", rx_dma->chan);
		} else
			printk("===>chan %d: request irq succeed!\n", rx_dma->chan);
	}
	...
	switch(pdev->id) {
		...
		case 2:
			...
			break;
		case 3:
			cpm_start_clock(CGM_I2C3);
			__gpio_as_i2c3();
			break;
		case 4:
			cpm_start_clock(CGM_I2C4);
			__gpio_as_i2c4_io0();
			//	__gpio_as_i2c4_io1();
			//	__gpio_as_i2c4_io2();
			break;
	...
	dev_info(&pdev->dev, "JZ4780 i2c bus driver.\n");
...
#endif
#ifdef CONFIG_JZ_I2C3
static struct platform_driver i2c_3_jz_driver = {
	.probe		= i2c_jz_probe,
	.remove		= i2c_jz_remove,
	.driver		= {
		.name	= "jz_i2c3"	
...
#endif
#ifdef CONFIG_JZ_I2C4
static struct platform_driver i2c_4_jz_driver = {
	.probe          = i2c_jz_probe,
	.remove         = i2c_jz_remove,
	.driver         = {
		.name   = "jz_i2c4"
...
#endif
#ifdef  CONFIG_JZ_I2C3
	ret = platform_driver_register(&i2c_3_jz_driver);
#endif
#ifdef  CONFIG_JZ_I2C4
	ret = platform_driver_register(&i2c_4_jz_driver);
#endif
	return ret;
}
...
#endif
#ifdef  CONFIG_JZ_I2C3
	platform_driver_unregister(&i2c_3_jz_driver);
#endif
#ifdef  CONFIG_JZ_I2C4
	platform_driver_unregister(&i2c_4_jz_driver);
#endif
