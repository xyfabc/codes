...
unsigned char *my_frm_buf[FRM_BUF_NUM];	/*current trans buffer pointer*/
...
extern void sensor_set_i2c_speed(struct i2c_client *, unsigned long);

int sensor_read_reg2(struct i2c_client *client, unsigned char reg,unsigned char *value);

int sensor_read_all_regs(void);

int sensor_write_reg2(struct i2c_client *client, unsigned char reg, unsigned char val);
...
struct JZ_OV7725_dmaqueue  dma_que;
...
static int cim_select_cpu(void)
{
	static int sim_slecte = 0;
	//static int cim_tran_id = 0;
	int i;
	//u32 state = REG_CIM_STATE;
		
	if (REG_CIM_STATE & CIM_STATE_DMA_EOF) 
	{	
		if (sim_slecte == CIM_RGB)/*for RGB*/
		{
			...
			__cim_set_da(virt_to_phys(cim_frame_desc[rgb_read_cim*2+1]));
		...
		{		
			...
			__cim_set_da(virt_to_phys(cim_frame_desc[yuv_read_cim*2]));
		}


		REG_CIM_STATE &= ~CIM_STATE_DMA_EOF;

		golf_cim_reset_rxfifo();	// resetting rxfifo
		golf_cim_unreset_rxfifo();
		golf_cim_enable_dma();	// enable dma
		__cim_enable();
...
static void thcim_start(void)
{
	...
	__cim_set_da(virt_to_phys(cim_frame_desc[0]));
...
static irqreturn_t cim_irq_handler(int irq, void *dev_id)
{
	...
	int i;
	unsigned int state = REG_CIM_STATE;
	unsigned long flags;
...
static int JZ_GOV7725_open(struct inode *inode, struct file *file)
{
...
static int JZ_GOV7725_release(struct inode *inode, struct file *file)
//static int JZ_GOV7725_release(struct file *file)
{
	int minor = iminor(inode);
	printk("JZ_GOV7725: release called  minor: %d \n", minor);
...
static int JZ_GOV7725_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
	switch (cmd) {
	case VIDIOC_G_FMT:
	{
	...
	case VIDIOC_S_EXPOSURE:
	{
		...
		mdelay(30);
		ret = set_sensor_reg(iminor(inode),AECH, h_data);
		mdelay(30);
		ret = set_sensor_reg(iminor(inode),AECL, l_data);
	...
	case VIDIOC_G_EXPOSURE:
		{
			...
			get_sensor_reg(iminor(inode), AECL, &l_data);
			mdelay(10);
			get_sensor_reg(iminor(inode), AECH, &h_data);
...
#if defined(CONFIG_SOC_JZ4760B) || defined(CONFIG_SOC_JZ4770)
	cim_cfg.cfg |= CIM_CFG_DMA_BURST_INCR32  | (0<<CIM_CFG_RXF_TRIG_BIT);// (n+1)*burst = 2*16 = 32 <64
...
//	REG_CIM_OFFSET = c->offs;
	REG_CIM_CFG		= 0x00024c46/*0x00024c56*/;  // 0,1,2,3,4,5,6,7
//	REG_CIM_CTRL	= 0x00004984;
	REG_CIM_SIZE	= 0x011001e0;
	REG_CIM_OFFSET	= 0x00680050;
...
static void cim_print_regs60(void)
{
	printk("REG_CIM_CFG \t= \t0x%08x\n", REG_CIM_CFG);
	printk("REG_CIM_CTRL \t= \t0x%08x\n", REG_CIM_CTRL);
#ifdef CONFIG_SOC_JZ4760B
...
{
#if defined(CONFIG_SOC_JZ4760B) || defined(CONFIG_JZ4760_F4760) || defined(CONFIG_JZ4810_F4810) || defined(CONFIG_SOC_JZ4770)
...
{
	cpm_stop_clock(CGM_CIM);
	cpm_set_clock(CGU_CIMCLK, 48000000);
...
static int ov77255_rgb_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	__gpio_as_output1(GPD(10));	// SENSOR RESET PIN
