...
#include <linux/proc_fs.h>		// add, yjt, 20130807
...
static int cim_select_cpu(void)
{
	...
	if (REG_CIM_STATE(0) & CIM_STATE_DMA_EOF) 
	{	
...
//up(&exp_sem);//add by dy 20130325
			...
			__cim_set_da(0, virt_to_phys(cim_frame_desc[rgb_read_cim*2+1]));
...
//down(&exp_sem);  //add by dy 20130325
			...
			__cim_set_da(0, virt_to_phys(cim_frame_desc[yuv_read_cim*2]));
		}


		REG_CIM_STATE(0) &= ~CIM_STATE_DMA_EOF;

		golf_cim_reset_rxfifo(0);	// resetting rxfifo
		golf_cim_unreset_rxfifo(0);
		golf_cim_enable_dma(0);	// enable dma
		__cim_enable(0);
...
//down(&exp_sem);//add by dy 20130325
		...
		//long time;		// del, yjt, 20130725
		//time = interruptible_sleep_on_timeout(&wq_out_yuv,1000);  //add by dy 20130517		// del, yjt, 20130725

		i2c_pin_cim_yuv();	// add, yjt, 20130725
		mdelay(10);			// add, yjt, 20130725
...
//	REG_CIM_CTRL(0)	= 0x00004984;
	...
	REG_CIM_OFFSET(0)	= 0x00680050;
		...
		//__cim_disable_bypass_func(0);

		__cim_enable_auto_priority(0);
		//__cim_disable_auto_priority(0);

		__cim_enable_emergency(0);
		//__cim_disable_emergency(0);
...
static int i2c_pin_cim_select(struct file *file, const char *buffer, unsigned long count, void *data)
{
	...
	{
		printk("illegal selection (%d), expect 0(rgb) or 1(yuv)!\n", l);
	}
...
static int __init JZ_OV7725_init(void)
{
	...
	struct proc_dir_entry *res;		// add, yjt, 20130807
/*
