/*
 * linux/drivers/media/video/jz4775_cim.c -- Ingenic Camera Controller driver
 *
...
MODULE_DESCRIPTION("V4L2 driver for Jz4775 CIM");
MODULE_AUTHOR("Yuan Yilong <ylyuan@ingenic.cn>");
MODULE_LICENSE("GPL");
...
static void cim_print_regs(struct cim_dev *cim)
{
	int id = cim->id;
	...
	printk("REG_CIM_OFFSET(%d) \t= \t0x%08x\n", id, REG_CIM_OFFSET(id));
	printk("REG_CIM_FS(%d) \t\t= \t0x%08x\n", id, REG_CIM_FS(id));
	printk("REG_CIM_YFA(%d) \t\t= \t0x%08x\n", id, REG_CIM_YFA(id));
	printk("REG_CIM_YCMD(%d) \t= \t0x%08x\n", id, REG_CIM_YCMD(id));
	printk("REG_CIM_CBFA(%d) \t= \t0x%08x\n", id, REG_CIM_CBFA(id));
	printk("REG_CIM_CBCMD(%d) \t= \t0x%08x\n", id, REG_CIM_CBCMD(id));
	printk("REG_CIM_CRFA(%d) \t= \t0x%08x\n", id, REG_CIM_CRFA(id));
	printk("REG_CIM_CRCMD(%d) \t= \t0x%08x\n", id, REG_CIM_CRCMD(id));
	printk("REG_CIM_TC(%d) \t\t= \t0x%08x\n", id, REG_CIM_TC(id));
	printk("REG_CIM_TINX(%d) \t= \t0x%08x\n", id, REG_CIM_TINX(id));
	printk("REG_CIM_TCNT(%d) \t= \t0x%08x\n", id, REG_CIM_TCNT(id));
	printk("=========================================\n");
}

static void cim_print_desc(struct cim_dev *cim, struct cim_desc *frame_desc)
{
	int i = 0;
...
//	int i;
//	for (i = 0; i < buf_cnt; i++)
//		cim_print_desc(cim, &cim->dma_desc[i]);
	...
	//enable cim
	__cim_set_da(cim->id, virt_to_phys(desc));
	__cim_clear_state(cim->id);	// clear state register
	__cim_reset_rxfifo(cim->id);	// resetting rxfifo
	__cim_unreset_rxfifo(cim->id);
	__cim_enable_dma(cim->id);
	__cim_enable(cim->id);
...
static void cim_set_fmt(struct cim_dev *cim, struct cim_fmt *fmt, __u32 priv)
{
	...
	//CIMCFG.DSM 
	__cim_enable_gated_clock_mode(id);	//Gate Clock Mode
	...
	__cim_input_data_format_select_YUV444(id);
#elif defined(CONFIG_VIDEO_CIM_IN_FMT_YUV422)
	__cim_input_data_format_select_YUV422(id);
#elif defined(CONFIG_VIDEO_CIM_IN_FMT_ITU656)
	__cim_input_data_format_select_ITU656(id);
#else
...
#error "Unsupported input format!"
...
#endif
	//CIMCFG.ORDER
	__cim_set_input_data_stream_order(id, 0); /* YCbCr or Y0CbY1Cr */
	...
	//CIMCFG.PACK
	__cim_set_data_packing_mode(id, packing); 
	//CIMCFG.SEP
	 ...
	 * Priority Control
	 */
	__cim_enable_auto_priority(id);

	__cim_enable_emergency(id);
	 ...
	 */
	__cim_set_opg(id, 1);
	__cim_enable_priority_control(id);
}

static void cim_config(struct cim_dev *cim)
{
	int id = cim->id;	/* cim0 or cim1 */

	REG_CIM_CFG(id)  = cim->regs.cfg;
	REG_CIM_CTRL(id) = cim->regs.ctrl;
	REG_CIM_SIZE(id) = cim->regs.size;
	REG_CIM_OFFSET(id) = cim->regs.offs;

#ifdef CIM_SAFE_DISABLE
	__cim_enable_vdd_intr(id);
#else
	__cim_disable_vdd_intr(id);
#endif

#ifdef CIM_INTR_SOF_EN
	__cim_enable_sof_intr(id);
#else
	__cim_disable_sof_intr(id);
#endif

#ifdef CIM_INTR_EOF_EN
	__cim_enable_eof_intr(id);
#else
	__cim_disable_eof_intr(id);
#endif

#ifdef CIM_INTR_STOP_EN
	__cim_enable_stop_intr(id);
#else
	__cim_disable_stop_intr(id);
#endif

#ifdef CIM_INTR_OF_EN
	__cim_enable_rxfifo_overflow_intr(id);
#else
	__cim_disable_rxfifo_overflow_intr(id);
#endif

#ifdef CIM_INTR_EEOF_EN
	__cim_set_eeof_line(id, 100);
	__cim_enable_eeof_intr(id);
#else
	__cim_set_eeof_line(id, 0);
	__cim_disable_eeof_intr(id);
#endif
...
static void cim_init_config(struct cim_dev *cim)
{
	...
	cim->regs.cfg &= ~CIM_CFG_EXPAND_MODE;		//pakage mode as default
	cim->regs.cfg &= ~CIM_CFG_DMA_BURST_TYPE_MASK;
	cim->regs.cfg |= CIM_CFG_DMA_BURST_INCR32;

	cim->regs.ctrl = CIM_CTRL_DMA_SYNC | CIM_CTRL_FRC_1;
}

static void cim_power_off(int id)
{
	if (id == 0)
...
static void cim_power_on(int id)
{
	if (id == 0) {
		cpm_stop_clock(CGM_CIM0);
#ifndef CONFIG_JZ_FPGA
		cpm_set_clock(CGU_CIMCLK,24000000);	/* Master Clock */
#endif
...
static void cim_stop(struct cim_dev *cim)
{
	...
	__cim_disable(cim->id);
	__cim_clear_state(cim->id);

	desc->ops->sensor_set_power(desc, 1);
	cim_power_off(cim->id);
}
...
//	struct cim_dmaqueue *vidq	= &dev->vidq;
	...
	state = state_back = REG_CIM_STATE(dev->id);
	...
	if (state & CIM_STATE_DMA_STOP) {
		__cim_disable(dev->id);
		__cim_reset_rxfifo(dev->id);
		__cim_unreset_rxfifo(dev->id);
		__cim_clear_state(dev->id);	// clear state register
	...
	if ( 	(state & CIM_STATE_RXF_OF)
		...
		REG_CIM_STATE(dev->id) &= ~CIM_STATE_VDD;
		__cim_disable(dev->id);
		__cim_reset_rxfifo(dev->id);
		__cim_unreset_rxfifo(dev->id);
		__cim_clear_state(dev->id);	// clear state register
...
buffer_setup(struct videobuf_queue *vq, unsigned int *count, unsigned int *size)
{
	...
	*size = fh->width*fh->height*3;		// => videobuf_buffer.bsize
						...
						enum v4l2_field field)
{
	...
	buf->vb.size = fh->width*fh->height*3;
...
static int cim_enable_image_mode(struct cim_dev *cim, int image_w,int image_h,int width,int height)
{
	...
	__cim_set_line(cim->id, image_h);
	__cim_set_pixel(cim->id, half_words_per_line);

	__cim_set_v_offset(cim->id, voffset);
	__cim_set_h_offset(cim->id, hoffset);

	__cim_enable_size_func(cim->id);
...
{
	__cim_disable_size_func(cim->id);
