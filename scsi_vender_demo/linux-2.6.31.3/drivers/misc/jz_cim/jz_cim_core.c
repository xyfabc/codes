...
void cim_dump_tlb(void);
unsigned int cim_refill_tlb(unsigned int vaddr);
unsigned int cim_get_tlb_entry(unsigned int vaddr);
unsigned int get_phy_addr(unsigned int vaddr, int pid);
void cim_read_tlb_content(int idx);
...
//	imgsize = cur_desc->preview_parm.width * (cur_desc->preview_parm.height-1);
	...
	if (jz_cim->cim_cfg.packed)
		...
		case CIM_OUT_FMT_YUV420:
			...
			capture_desc.cr_frame = base_paddr + imgsize + 64;
#else
...
static void cim_config(void)
{
	...
	REG_CIM_CFG  = c->cfg;	//coming from ov3640_sensor_desc and cim_init_config()
	REG_CIM_CTRL = c->ctrl;
	REG_CIM_SIZE = c->size;
...
#error "Unsupported input format!"
	...
	__cim_set_data_packing_mode(4); /* 0x 44 33 22 11 or 0x Cr Y1 Cb Y0 */	/* for YUV444_YUV */
#endif
	...
	__cim_csc_yuv420();
#else
	__cim_enable_bypass_func();	//default mode for jz4770 & jz4780
#endif
	...
	__cim_enable_framesize_check_error_intr();
#endif
...
static int cim_release(struct inode *inode, struct file *filp)
{
	__cim_sw_reset();	/* Reset all registers(Reset Value). */
	cur_desc->scale = NULL;
	cim_fb_free();
...
//	pgdir=pgd_offset(current->mm,vaddr);
	...
	if (testflag)
		printk("==>%s() L%d: pgdir=%p\n", __func__, __LINE__, pgdir);
#ifdef CONFIG_PGTABLE_4
...
#endif
	if (testflag)
		printk("==>%s() L%d: pmdir=%p\n", __func__, __LINE__, pmdir);

	pte=pte_offset(pmdir,vaddr);
	if (testflag)
		printk("==>%s() L%d: pte=%p, pte_val=0x%08x\n", __func__, __LINE__, pte, pte_val(*pte));
...
{
	printk("==>%s() L%d\n", __func__, __LINE__);
	__cim_set_tlb_idx(idx);
	printk("==>%s: Index=%d, Content=0x%08x\n", __func__, REG_CIM_TINX, REG_CIM_TCNT);
}

void cim_read_tlb_all(void)
{
	int idx;
	for (idx=0; idx<8; idx++) {
...
//	tba = ((unsigned int)&tlb_entry[0] - vpn * 4) >> 2;	//word aligned
	...
	__cim_set_tlb_addr(tba);
	__cim_enable_tlb_error_intr();
	__cim_reset_tlb();
	__cim_unreset_tlb();
	__cim_enable_tlb();
}
...
//	struct cim_tlb_entry *entry = (struct cim_tlb_entry *)((REG_CIM_TC & ~0x3) + vpn * 4);
	struct cim_tlb_entry *entry = (struct cim_tlb_entry *)(CKSEG1ADDR(REG_CIM_TC & ~0x3) + vpn * 4);
	unsigned int paddr = 0;
...
//	fs_w = 319;	//320 - 8 + 7
	...
	*/
	//set frame size
	__cim_set_fs_v_size(fs_h-1);	//vertical size
	__cim_set_fs_h_size(fs_w-1);	//horizontal size
	__cim_set_fs_bpp((fs_bpp>>3)-1);	//Bytes per pixel
	if (!cur_desc->scale)
		__cim_enable_framesize_check();
#ifdef CONFIG_JZ_CIM_ARIF
	__cim_enable_arif();	//enable auto recovery for incomplete frame
...
static int cim_set_buffer(void __user *argp, struct camera_buffer_info *binfo)
{
	...
	binfo->base_paddr = get_phy_addr(binfo->base_vaddr);
...
//	printk("==>%s() L%d\n", __func__, __LINE__);
	...
	if (binfo->base_vaddr & (PAGE_SIZE - 1))	//page aligned or not
		return -EINVAL;
//	printk("==>%s() L%d: vaddr=0x%08x\n", __func__, __LINE__, binfo->base_vaddr);
...
static int cim_do_scale(struct cim_scale *s)
{
	...
	__cim_set_h_down_scale(hs);
	__cim_set_v_down_scale(vs);

	if (s->ignore) {
		__cim_ignore_h_frame_reminder();
		__cim_ignore_v_frame_reminder();
	} else {
		REG_CIM_CTRL2 &= ~( CIM_CTRL2_FRAGHE | CIM_CTRL2_FRAGVE );
	}
...
static int __init cim_init(void)
{
	...
	printk("PAGE_OFFSET:%lld\n", PAGE_OFFSET);
	printk("PHYS_OFFSET:%d\n", PHYS_OFFSET);
