...
/******/spin_unlock_irqrestore(&fresh_lock,flags);/*******************************************************/
	...
	dprintk("-->%s L%d: buf_offsize_app_drv=0x%08x\n", __func__, __LINE__, buf_offsize_app_drv);
...
//	if ((ret = request_irq(IRQ_CIM, cim_irq_handler, IRQF_DISABLED, CIM_NAME, jz_cim))) {
	if ((ret = request_irq(30-cim_id*2, cim_irq_handler, IRQF_DISABLED, CIM_NAME, jz_cim))) {
		...
		printk(KERN_ERR "CIM0 could not get IRQ\n");
		goto out;
	}
