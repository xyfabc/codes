...
#include "ov3640_set.c"
#include "ov3640_set_mode.c"
...
static struct camera_sensor_desc ov3640_sensor_desc;
...
#elif defined(CONFIG_JZ4775_F4775) || defined(CONFIG_JZ4780_F4780)
//#define GPIO_CAMERA_RST		(32*1+9)	/* GPB9 */
#define GPIO_CAMERA_RST		(32*6+9)	/* GPG9 */
#elif defined(CONFIG_JZ4775_ORION)
#define GPIO_CAMERA_RST		(32*0+27)	/* GPA27, for Jz4775 Orion Board CIM1 */
#elif defined(CONFIG_JZ4780_NORMA)
#define GPIO_CAMERA_RST		(32*5+14)	/* GPF14, for Jz4780 Norma Board */
#else
#error "ov3640/ov3640_camera.c , please define camera for your board."
#endif

static void ov3640_power_down(void)
{
...
#elif defined(CONFIG_JZ4770_NORMA)
//	__gpio_as_output1(GPD(8));
#endif
	mdelay(5);
}

static void ov3640_power_up(void)
{
	dprintk("=======%s:%d\n", __FUNCTION__, __LINE__);
#if defined(CONFIG_JZ4750_AQUILA)
	__gpio_as_output(4*32+23);
...
#elif defined(CONFIG_JZ4770_NORMA)
//	__gpio_as_output0(GPIO_CAMERA_PWDN_03M);	//for Jz4780 T760
//	__gpio_as_output0(GPIO_CAMERA_PWDN_2M);		//for Jz4780 T760
#endif
	mdelay(5);
}
...
#else
	dprintk("=======%s:%d\n", __FUNCTION__, __LINE__);
#if defined(CONFIG_JZ4810_F4810) || defined(CONFIG_JZ4770_F4770) || defined(CONFIG_JZ4770_PISCES) || defined(CONFIG_JZ4775_F4775) || defined(CONFIG_JZ4780_F4780)
	//while(1)
	{
...
static int ov3640_set_balance(balance_flag_t balance_flag,int arg)
{
...
static int ov3640_set_antibanding(antibanding_flag_t antibanding_flag,int arg)
{
...
static int ov3640_set_flash_mode(flash_mode_flag_t flash_mode_flag,int arg)
{
...
static int ov3640_set_scene_mode(scene_mode_flag_t scene_mode_flag,int arg)
{
...
static int ov3640_set_focus_mode(focus_mode_flag_t flash_mode_flag,int arg)
{
#if 0
...
static int ov3640_set_power(int state)
{
	dprintk("=======%s:%d\n", __FUNCTION__, __LINE__);
	switch (state)
	{
 ...
 * 2: recording
 */
static int ov3640_set_effect(effect_flag_t effect_flag,int arg)
{
...
static int ov3640_set_output_format(pixel_format_flag_t pixel_format_flag,int arg)
{
	dprintk("=======%s:%d\n", __FUNCTION__, __LINE__);
...
static int ov3640_set_function(int function,void *cookie)
{
	dprintk("=======%s:%d\n", __FUNCTION__, __LINE__);
	switch (function)
	{
	...
	case 2:
		break;
	case 3:
		//ov3640_do_focus(ov3640_sensor_desc.client);
		break;
	case 4:
...
static int ov3640_set_fps(int fps)
{
	dprintk("=======%s:%d\n", __FUNCTION__, __LINE__);
...
static int ov3640_set_night_mode(int enable)
{
	dprintk("=======%s:%d\n", __FUNCTION__, __LINE__);
	if(enable)
...
static int ov3640_set_luma_adaptation(int arg)
{
	dprintk("luma_adaptation : %d",arg);
	return 0;
}

static int ov3640_set_parameter(int cmd, int mode, int arg)
{
	dprintk("=======%s:%d\n", __FUNCTION__, __LINE__);
	switch(cmd)
	{
...
static int ov3640_sensor_init(void)
{
	dprintk("=======%s:%d\n", __FUNCTION__, __LINE__);
	ov3640_set_mclk(0);
	ov3640_reset();
...
static int ov3640_sensor_probe(void)
{
	int retval = 0;
...
#if 0
	while (1) {
	retval = sensor_read_reg16(ov3640_sensor_desc.client, 0x300a);	//read product id MSBs
	...
	if(retval == 0x36)	//ov3640 product id is 0x36
		return 0;
	else {
...
static int ov3640_set_resolution(int width,int height,int bpp,pixel_format_flag_t fmt,camera_mode_t mode)
{
...
#endif
	camera_sensor_register_1(&ov3640_sensor_desc);

	printk("OV3640-1 probed.\n");
	return 0;
}

static struct camera_sensor_ops ov3640_sensor_ops = {
