...
void jz_mmc_stop_dma(struct jz_mmc_host *host)
{
	...
	while (REG_DMAC_DCCSR(host->dma.channel) & DMAC_DCCSR_EN)
		;
}
...
void dump_dma_desc(JZ_MSC_DMA_DESC *desc, int line){
	printk("--------------line:%d-------------------\n",line);
	printk("dcmd:%08x\n",desc->dcmd);
	printk("dsadr:%08x\n",desc->dsadr);
	printk("dtadr:%08x\n",desc->dtadr);
...
int jz_select_burst(int len, JZ_MSC_DMA_DESC *desc)
{
	int ds;
...
static int sg_to_desc(struct scatterlist *sgentry, JZ_MSC_DMA_DESC *first_desc,
		      int *desc_pos, int mode, int ctrl_id, struct jz_mmc_host *host)
{
...
//	unsigned int ds;
	...
	//ds = jz_select_burst(dma_len, desc);
	//printk("++++++ select burst = %d\n", ds);
	...
	//max_burst_len = dma_len & (~31);
	//max_burst_len = dma_len & (~15);

	burst4_dma_addr = max_burst_dma_addr + max_burst_len;
	burst4_len = dma_len & 63;
	//burst4_len = dma_len & 31;
	//burst4_len = dma_len & 15;
	...
	if (max_burst_len) {
		...
		//desc->ddadr = (next << 24) | (max_burst_len / 16) ;
...
//	REG_DMAC_DMACR |= DMAC_DMACR_FMSC;
...
#if 0
	host->dma_desc = (unsigned int)host->dma_desc | 0xa0000000;
	tmp_buf = (char *)__get_free_pages(GFP_KERNEL, 5);
	tmp_buf = (unsigned int)tmp_buf | 0xa0000000;
#endif
