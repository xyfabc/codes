...
unsigned int get_addr_from_name(char *name, struct jz_mmc_platform_data *pdata){
	...
	printk("Please check board-pisces.c , get the right partition name\n");
	return -1;
}
...
static void jz_mmc_request(struct mmc_host *mmc, struct mmc_request *mrq)
{
	...
	unsigned int tm[2] = {0};	// add, yjt, 20130607
	...
	host->curr_mrq = mrq;
	//tm[0] = jiffies_to_msecs(jiffies);		// add, yjt, 20130607
	functions->execute_cmd(host);
#if 0
	tm[1] = jiffies_to_msecs(jiffies);		// add, yjt, 20130607
	printk("tm[0] -> tm[1], [%u -> %u]\n", tm[0], tm[1]);
	diff += tm[1] - tm[0];
	times++;
	printk("diff=%d, times=%d\n", diff, times);
#endif
	jz_mmc_finish_request(host, mrq);
}
