/*
 * linux/drivers/mtd/nand/jz4780_nand_ubi.c
 *
 * JZ4780 NAND driver
 *
...
{
	__dmac_channel_programmable_clear();
...
static int jz4780_nand_pdma_init(struct mtd_info *mtd)
{
	struct nand_chip *chip = mtd->priv;
	struct pdma_msg	msg;
...
#if defined(CONFIG_MTD_NAND_DMA)
	printk ("Init DMA MCU:\n");
	ret = jz4780_nand_pdma_init(jz_mtd);
	if (ret)
		goto invalid_pdma;
#endif
