 ...
 */
static void nand_release_device(struct mtd_info *mtd, int select)
{
...
static void nand_select_chip(struct mtd_info *mtd, int chipnr)
{
	...
	case 3:
		chip->cmd_ctrl(mtd, NAND_CMD_NONE, NAND_NCE |
			TNAND_NCE_SET | NAND_CTRL_CHANGE);
	}
#endif
}
...
#ifdef CONFIG_MTD_NAND_TOGGLE
		chip->cmd_ctrl(mtd, NAND_CMD_NONE, NAND_NCE |
				NAND_FCE_SET | NAND_CTRL_CHANGE);
#endif
		chip->cmdfunc(mtd, NAND_CMD_READOOB, chip->badblockpos, page);
#ifdef CONFIG_MTD_NAND_TOGGLE
		chip->cmd_ctrl(mtd, NAND_CMD_NONE, NAND_NCE |
				TNAND_READ_PERFORM | NAND_CTRL_CHANGE);
#endif
...
#ifdef CONFIG_MTD_NAND_TOGGLE
		blank = chip->read_byte(mtd);
		chip->cmd_ctrl(mtd, NAND_CMD_NONE, NAND_NCE |
				NAND_FCE_CLEAN | NAND_CTRL_CHANGE);
#endif
	}

	if (getchip)
		nand_release_device(mtd, FL_SELECT);
...
static int nand_check_wp(struct mtd_info *mtd)
{
	...
	blank = chip->read_byte(mtd);
#endif
	status = chip->read_byte(mtd) & NAND_STATUS_WP? 0 : 1;
#ifdef CONFIG_MTD_NAND_TOGGLE
	chip->cmd_ctrl(mtd, NAND_CMD_NONE, NAND_NCE |
			NAND_FCE_CLEAN | NAND_CTRL_CHANGE);
#endif
			    ...
			    int column, int page_addr)
{
	...
	if (column != -1 || page_addr != -1) {
		int ctrl = NAND_NCE | NAND_ALE | NAND_CTRL_CHANGE;
...
#if !(defined(CONFIG_MTD_NAND_DMA) && defined(CONFIG_MTD_NAND_FIRMWARE))
	chip->select_chip(mtd, chipnr);
#else
	chip->chip_num = chipnr + 1; /* For Jz NAND firmware */
#endif
		     ...
		     size_mtd_t *retlen, uint8_t *buf)
{
	...
	*retlen = chip->ops.retlen;
	...
	nand_release_device(mtd, FL_SELECT);
#else
	nand_release_device(mtd, FL_NOSELECT);
#endif
...
#ifdef CONFIG_MTD_NAND_TOGGLE
	chip->cmd_ctrl(mtd, NAND_CMD_NONE, NAND_NCE |
			TNAND_WRITE_PERFORM | NAND_CTRL_CHANGE);
#endif
	/* Write OOB data */
			 ...
			 struct mtd_oob_ops *ops)
{
 ...
 out:
	nand_release_device(mtd, select);
	return ret;
}
...
{
	int status;
#if defined(CONFIG_MTD_NAND_TOGGLE) && defined(CONFIG_MTD_NAND_DMA)
	u8 blank;
#endif
	...
	if (!cached || !(chip->options & NAND_CACHEPRG)) {
		/*
		 *  __nand_cmd(CMD_PAGEPROG) and __nand_sync() have been done
		 * by DMA for jz4750 and later chip, status should still be read
		 * by "status = chip->waitfunc(mtd, chip)"
		 */
...
#else
	/* Select the NAND device */
	chip->select_chip(mtd, chip->chip_num - 1);
#ifdef CONFIG_MTD_NAND_TOGGLE
	chip->cmd_ctrl(mtd, NAND_CMD_NONE, NAND_NCE |
			NAND_FCE_SET | NAND_CTRL_CHANGE);
#endif
	...
	chip->cmd_ctrl(mtd, NAND_CMD_NONE, NAND_NCE |
			TNAND_READ_PERFORM | NAND_CTRL_CHANGE);
#endif
	ret = chip->verify_buf(mtd, buf, mtd->writesize);
#ifdef CONFIG_MTD_NAND_TOGGLE
	chip->cmd_ctrl(mtd, NAND_CMD_NONE, NAND_NCE |
			NAND_FCE_CLEAN | NAND_CTRL_CHANGE);
#endif
			     ...
			     struct mtd_oob_ops *ops)
{
	...
	printk("nand_write_oob: to = 0x%08x, len = %i\n",
	      (unsigned int)to, (int)ops->ooblen);
...
//	printk("nand_erase: start = 0x%012llx, len = %llu\n", (unsigned long long)instr->addr, (unsigned long long)instr->len);
...
//	for(i = 0; i < EXT_ID_NUM; i++)
//		ext_id[i] = chip->read_byte(mtd);
#endif
...
int nand_scan_tail(struct mtd_info *mtd)
{
	...
	if (!(chip->options & NAND_OWN_BUFFERS)) {
		buffer_size = sizeof(struct nand_buffers);
		buffer_size += mtd->writesize + mtd->oobsize * 3 + chip->ecc.size * 2;
		chip->buffers = (struct nand_buffers *)kmalloc(buffer_size, GFP_KERNEL);

		if (chip->buffers) {
			chip->buffers->databuf = (uint8_t *)(chip->buffers + 1);
			chip->buffers->ecccalc = chip->buffers->databuf + mtd->writesize + mtd->oobsize;
			chip->buffers->ecccode = chip->buffers->ecccalc + mtd->oobsize + chip->ecc.size;
		}
	}
