...
static DEFINE_SPINLOCK(jz4775_rtc_lock);
...
#if 0
static int jz4775_rtc_read_callback(struct device *dev, int data)
{
	if (data & RTC_PF) {
...
static int jz4775_rtc_open(struct device *dev)
{
	...
	ret = request_irq(IRQ_RTC, jz4775_rtc_interrupt, IRQF_DISABLED,
...
{
	spin_lock_irq(&jz4775_rtc_lock);

	spin_unlock_irq(&jz4775_rtc_lock);
...
static const struct rtc_class_ops jz4775_rtc_ops = {
	...
	//.read_callback = jz4775_rtc_read_callback,
...
static int jz4775_rtc_probe(struct platform_device *pdev)
{
	...
	if((hspr != cfc) || (rgr_1hz != RTC_FREQ_DIVIDER))
	{
		...
		printk("jz4775-rtc: rtc status reset by power-on\n");
		/* Set 32768 rtc clocks per seconds */
		rtc_write_reg(RTC_RTCGR, RTC_FREQ_DIVIDER | (0 << 16));
		...
		jz4775_rtc_set_time(NULL, &default_tm);
...
//		rtc_write_reg(RTC_RTCCR, RTC_RTCCR_RTCE | RTC_RTCCR_AE | RTC_RTCCR_AIE);
	...
	rtc = rtc_device_register(pdev->name, &pdev->dev, &jz4775_rtc_ops,
				THIS_MODULE);
...
//	rtc_write_reg(RTC_CKPCR, 1 << 4 | 3 << 1 | 0);  //output by pd14
	rtc_write_reg(RTC_PWRONCR, 1 | rtc_read_reg(RTC_PWRONCR));
