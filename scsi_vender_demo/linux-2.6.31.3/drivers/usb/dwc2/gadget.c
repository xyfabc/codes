...
static int __dwc2_gadget_ep_queue(struct dwc2_ep *dep, struct dwc2_request *req)
{
	...
	if (likely(r->length) && (unsigned int)(r->buf) % 4 == 0) {
...
//	dma_set_coherent_mask(&dwc->gadget.dev, dwc->dev->coherent_dma_mask);
	if (!dma_supported(&(dwc->gadget.dev), dwc->dev->coherent_dma_mask))
		(&(dwc->gadget.dev))->coherent_dma_mask = dwc->dev->coherent_dma_mask;
...
//	ret = bind(&dwc->gadget);
	...
	if (likely(dwc2_is_device_mode(dwc))) {
		HANVON_DBG("dwc2_is_device_mode");
		/* default to 64 */
		dwc2_gadget_ep0_desc.wMaxPacketSize = cpu_to_le16(64);
	...
	printk("++++++++++++++++++++++usb_gadget_register_driver+++++++++++done++++++++++++++++++++++++++++\n");
