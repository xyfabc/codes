...
#define cpm_inl(reg)  (REG32(reg))
#define cpm_outl(num, reg)    (REG32(reg)=num)
#define cpm_clear_bit(val,reg)  do{cpm_outl((cpm_inl(reg) & (~(val))),reg);}while(0)
#define cpm_set_bit(val,reg)    do{cpm_outl((cpm_inl(reg) |  (val)),reg);}while(0)
#define cpm_test_bit(val,reg)   (cpm_inl(reg) & (val))
...
#define gpio_get_value(pin)     __gpio_get_pin(pin)
...
//	struct va_format vaf;
	...
	printk("CPU%d: DWC2(%s): ", smp_processor_id(), comp);
