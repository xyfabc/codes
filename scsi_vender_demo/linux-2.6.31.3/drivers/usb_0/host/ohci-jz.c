...
#elif defined(CONFIG_SOC_JZ4775)
static void jz_start_ohc(struct platform_device *dev)
{
	printk(KERN_DEBUG __FILE__
		": starting JZ OHCI USB Controller\n");
	...
	/* 24M clock frequency */
	/* fixme : need support various external crystal //twxie */
	REG_CPM_USBPCR1 &= ~USBPCR1_REFCLKDIV_MASK;
	REG_CPM_USBPCR1 |= 1 << 24;

	/* Configurate UHC IBSOP */
	REG_CPM_USBPCR1 &= ~(7 << 14);
	REG_CPM_USBPCR1 |= (1 << 14);

	/* Configurate UHC XP */
	REG_CPM_USBPCR1 &= ~(3 << 12);
	...
	/* Enable ohci phy power */
	REG_CPM_USBPCR1 |= (1 << 17);;

	/* need otg PHY support clk */
