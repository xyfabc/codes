...
#if defined(CONFIG_JZ4770_SLCD_FORISE_BL28021)

#define WR_GRAM_CMD	0x0022

#if defined(CONFIG_JZ4775_F4775)
	#define PIN_CS_N 	(32 * 2 + 11)	// Chip select 	//GPc11;
	#define PIN_RESET_N 	(32 * 0 + 0)	/* LCD_REV GPF6 nor data0 */
	...
	#define PIN_CS_N 	(32 * 2 + 11)		// modify, yjt, 20130513
	#define PIN_RESET_N 	(32 * 2 + 10)	// modify, yjt, 20130513
#else
...
//		 .cfg = LCD_CFG_LCDPIN_SLCD | LCD_CFG_RECOVER | /* Underrun recover*/
	...
	.osd = {
		 ...
		 .fg0 = {24, 0, 0, 240, 320}, /* bpp, x, y, w, h */
		 .fg1 = {30, 0, 0, 240, 320}, /* bpp, x, y, w, h */
	 }
#else
	.panel = {
//		 .cfg = LCD_CFG_LCDPIN_SLCD | LCD_CFG_RECOVER | /* Underrun recover*/
...
static void Mcupanel_RegSet(unsigned int cmd, unsigned int data)
{
	...
	//printk("Mcupanel RegSet\n");
        ...
        if(timecnt1 >= timeout || timecnt1 >= timeout)
                printk("SLCD Mcupanel_RegSet timeout!!!\n");
}
...
static void Mcupanel_Command(unsigned int cmd) 
{
	unsigned int timecnt = 0;
        ...
        if(timecnt >= timeout)
                printk("SLCD Mcupanel_Command timeout!!!\n");
}
...
{     
	...
	Mcupanel_RegSet(0x00EB, 0x0020);	
	Mcupanel_RegSet(0x00EC, 0x003C);	
	Mcupanel_RegSet(0x00ED, 0x00C4);	
	Mcupanel_RegSet(0x00E8, 0x0048);	
	Mcupanel_RegSet(0x00E9, 0x0038);	
	Mcupanel_RegSet(0x00F1, 0x0001);	
	Mcupanel_RegSet(0x00F2, 0x0008);	
	Mcupanel_RegSet(0x0040, 0x0001);	
	Mcupanel_RegSet(0x0041, 0x0007);	
	Mcupanel_RegSet(0x0042, 0x0009);	
	Mcupanel_RegSet(0x0043, 0x0019);	
	Mcupanel_RegSet(0x0044, 0x0017);	
	Mcupanel_RegSet(0x0045, 0x0020);	
	Mcupanel_RegSet(0x0046, 0x0018);	
	Mcupanel_RegSet(0x0047, 0x0061);	
	Mcupanel_RegSet(0x0048, 0x0000);	
	Mcupanel_RegSet(0x0049, 0x0010);	
	Mcupanel_RegSet(0x004A, 0x0017);	
	Mcupanel_RegSet(0x004B, 0x0019);	
	Mcupanel_RegSet(0x004C, 0x0014);	
	Mcupanel_RegSet(0x0050, 0x001F);	
	Mcupanel_RegSet(0x0051, 0x0028);	
	Mcupanel_RegSet(0x0052, 0x0026);	
	Mcupanel_RegSet(0x0053, 0x0036);	
	Mcupanel_RegSet(0x0054, 0x0038);	
	Mcupanel_RegSet(0x0055, 0x003E);	
	Mcupanel_RegSet(0x0056, 0x001E);	
	Mcupanel_RegSet(0x0057, 0x0067);	
	Mcupanel_RegSet(0x0058, 0x000B);	
	Mcupanel_RegSet(0x0059, 0x0006);	
	Mcupanel_RegSet(0x005A, 0x0008);	
	Mcupanel_RegSet(0x005B, 0x000F);	
	Mcupanel_RegSet(0x005C, 0x001F);	
	Mcupanel_RegSet(0x005D, 0x00CC);	
	Mcupanel_RegSet(0x001B, 0x001B);	
	Mcupanel_RegSet(0x001A, 0x0001);	
	Mcupanel_RegSet(0x0024, 0x0072);	
	Mcupanel_RegSet(0x0025, 0x006A);	
	Mcupanel_RegSet(0x0023, 0x0083);	
	Mcupanel_RegSet(0x0018, 0x0036);	
	Mcupanel_RegSet(0x0019, 0x0001);	
	Mcupanel_RegSet(0x0001, 0x0000);	
	Mcupanel_RegSet(0x001F, 0x0088);	
	mdelay(5);	
...
void display_panel_init(void *ptr)
{
	...
	slcd_panel_init();
#else
	Mcupanel_Command(0x0022);	// slcd was initialized in uboot already, here we'd better repoint to (0, 0);
#endif
}
