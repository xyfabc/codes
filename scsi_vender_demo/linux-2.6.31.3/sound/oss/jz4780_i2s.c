...
#define DMA_ID_I2S0_TX			DMA_ID_AIC0_TX 
#define DMA_ID_I2S0_RX		        DMA_ID_AIC0_RX	
#define DMA_ID_I2S1_TX			DMA_ID_AIC1_TX
#define DMA_ID_I2S1_RX			DMA_ID_AIC1_RX

// reference to dma.c
#define DMA_AIC0_TX_CHAN			6
#define DMA_AIC0_RX_CHAN			7
//#define DMA_AIC1_TX_CHAN			4
//#define DMA_AIC1_RX_CHAN			5
...
#define PDMA_DSAR(n)  (PDMA_BASE + 0x00 + ((n) * 0x20))
#define PDMA_DTAR(n)  (PDMA_BASE + 0x04 + ((n) * 0x20))
#define PDMA_DTCR(n)  (PDMA_BASE + 0x08 + ((n) * 0x20))
#define PDMA_DRSR(n)  (PDMA_BASE + 0x0c + ((n) * 0x20))
#define PDMA_DCCSR(n)  (PDMA_BASE + 0x10 + ((n) * 0x20))
#define PDMA_DCMD(n)  (PDMA_BASE + 0x14 + ((n) * 0x20))
#define PDMA_DDAR(n)  (PDMA_BASE + 0x18 + ((n) * 0x20))
#define PDMA_DSDR(n)  (PDMA_BASE + 0x1c + ((n) * 0x20))

#define PDMA_DMACR  	(PDMA_BASE + 0x1000)              /* DMA control register */
#define PDMA_DMAIRQR     (PDMA_BASE + 0x1000 + 0x04)              /* DMA interrupt pending */
#define PDMA_DMADDBR     (PDMA_BASE + 0x1000 + 0x08)              /* DMA doorbell */
#define PDMA_DMADDSR     (PDMA_BASE + 0x1000 + 0x0C)              /* DMA doorbell set */

#define REG_PDMA_DSAR(n)        REG32(PDMA_DSAR((n)))
#define REG_PDMA_DTAR(n)        REG32(PDMA_DTAR((n)))
#define REG_PDMA_DTCR(n)        REG32(PDMA_DTCR((n)))
#define REG_PDMA_DRSR(n)        REG32(PDMA_DRSR((n)))
#define REG_PDMA_DCCSR(n)       REG32(PDMA_DCCSR((n)))
#define REG_PDMA_DCMD(n)        REG32(PDMA_DCMD((n)))
#define REG_PDMA_DDAR(n)        REG32(PDMA_DDAR((n)))
#define REG_PDMA_DSDR(n)        REG32(PDMA_DSDR((n)))
#define REG_PDMA_DMACR          REG32(PDMA_DMACR)
#define REG_PDMA_DMAIRQR        REG32(PDMA_DMAIRQR)
#define REG_PDMA_DMADDBR        REG32(PDMA_DMADDBR)
#define REG_PDMA_DMADDSR        REG32(PDMA_DMADDSR)

#define PDMAC_DRSR_RS_BIT	   0
#define PDMA_DRSR_RS_AIC1_OUT      (4 << PDMAC_DRSR_RS_BIT)
#define PDMA_DRSR_RS_AIC1_IN       (5 << PDMAC_DRSR_RS_BIT)
#define PDMA_DRSR_RS_AIC0_OUT      (6 << PDMAC_DRSR_RS_BIT)
#define PDMA_DRSR_RS_AIC0_IN       (7 << PDMAC_DRSR_RS_BIT)
...
//	*(unsigned int *)0xb342010c = 0x18;
	...
	printk("\tDRSR	= 0x%08x, addr = 0x%08x\n", REG_PDMA_DRSR(dmanr), PDMA_DRSR(dmanr));
	printk("\tDCCSR	= 0x%08x\n", REG_PDMA_DCCSR(dmanr));
	printk("\tDCMD	= 0x%08x\n", REG_PDMA_DCMD(dmanr));
}
#endif
...
static irqreturn_t jz_i2s_dma_irq (int irq, void *dev_id)
{
	...
	int dma_state = REG_PDMA_DCCSR(dma_chan);
	int err = 0;

	ENTER();

	REG_PDMA_DCCSR(dma_chan) = 0;
			      ...
			      unsigned long irqflags, void *irq_dev_id)
{
	...
	if (dev_id == DMA_ID_AIC0_TX) {
		i = DMA_AIC0_TX_CHAN;
		if (jz_dma_table[i].dev_id != DMA_ID_AIC0_TX) {
			BUG_ON(1);
		}
	} else if (dev_id == DMA_ID_AIC0_RX) {
		i = DMA_AIC0_RX_CHAN;
		if (jz_dma_table[i].dev_id != DMA_ID_AIC0_RX) {
			BUG_ON(1);
		}
	} else {
		BUG_ON(1);
...
*/
	...
	case DMA_ID_AIC0_TX:
		...
		chan->source	= PDMA_DRSR_RS_AIC0_OUT;
		break;
	case DMA_ID_AIC0_RX:
		chan->mode	= DMA_32BIT_RX_CMD | DMA_MODE_READ;
		chan->source	= PDMA_DRSR_RS_AIC0_IN;
		break;
	default:
...
static int init_audio_recorddma(audio_pipe *endpoint)
{
	...
	ENTER();
	if ((ch = jz_request_aic_dma(DMA_ID_I2S0_RX, "audio adc", jz_i2s_dma_irq, IRQF_DISABLED, endpoint)) < 0) {
		printk(KERN_ERR "%s: can't reqeust DMA DAC channel.\n", __FUNCTION__);
		return -1;
	}
	REG_PDMA_DMACR |= 1;
	REG_PDMA_DCMD(ch) =  DMAC_DCMD_DAI | DMAC_DCMD_SWDH_32 | DMAC_DCMD_TIE;
	REG_PDMA_DRSR(ch) =  PDMA_DRSR_RS_AIC0_IN;
	REG_PDMA_DSAR(ch) =  (unsigned int)CPHYSADDR(AIC_DR);

	endpoint->dma.ch = ch;
	endpoint->dma.trans_addr = (unsigned int *)PDMA_DTAR(ch);
	endpoint->dma.trans_count = (unsigned int *)PDMA_DTCR(ch);
	endpoint->dma.trans_mode = (unsigned int *)PDMA_DCMD(ch);
	endpoint->dma.data_addr = (unsigned int *)PDMA_DSAR(ch);

	endpoint->dma.rw = 0;
	printk("<===== zli audio record dma ======>\n");
	dump_dma(ch, __func__);
...
{
	int ch = 0;
	if ((ch = jz_request_aic_dma(DMA_ID_I2S0_TX,"audio dac", jz_i2s_dma_irq, IRQF_DISABLED, endpoint)) < 0) {
		printk(KERN_ERR "%s: can't reqeust DMA DAC channel.\n", __FUNCTION__);
		return -1;
	}

	REG_PDMA_DMACR |= 1;
	REG_PDMA_DCMD(ch) =  DMAC_DCMD_SAI | DMAC_DCMD_DWDH_32 | DMAC_DCMD_TIE;
	//printk("$$$$ before set --- REG_DMAC_DRSR(ch) = 0x%08x\n", REG_DMAC_DRSR(ch));
	REG_PDMA_DRSR(ch) =  PDMA_DRSR_RS_AIC0_OUT;
	//printk("$$$$ after force set --- REG_DMAC_DRSR(ch) = 0x%08x\n", REG_DMAC_DRSR(ch));
	REG_PDMA_DTAR(ch) =  (unsigned int)CPHYSADDR(AIC_DR);

	endpoint->dma.ch = ch;
	endpoint->dma.trans_addr = (unsigned int *)PDMA_DSAR(ch);
	endpoint->dma.trans_count = (unsigned int *)PDMA_DTCR(ch);
	endpoint->dma.trans_mode = (unsigned int *)PDMA_DCMD(ch);
	endpoint->dma.data_addr = (unsigned int *)PDMA_DTAR(ch);
	endpoint->dma.rw = 1;
	printk("<************* zli audio replay dma ************>\n");
	dump_dma(ch, __func__);
	return ch;
}
...
static inline int audio_trystart_dma_node(audio_dma_type* dma, audio_node *node)
{
	...
	if ((REG_PDMA_DCCSR(dma->ch) & DMAC_DCCSR_EN) == 0) {
		...
		*(dma->trans_count) = count * 8 / dma->onetrans_bit;
		REG_PDMA_DCCSR(dma->ch) = DMAC_DCCSR_NDES | DMAC_DCCSR_EN;
