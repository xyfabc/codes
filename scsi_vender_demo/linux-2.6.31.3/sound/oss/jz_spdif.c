...
static wait_queue_head_t    trans_queue;
static int     avialable_trans;
...
static inline void audio_start_dma(int chan, void *dev_id, unsigned long phyaddr,int count, int mode)
{
	...
	if(mode == DMA_MODE_WRITE){
		...
		REG_DMAC_DRSR(chan) = 0x06;
		REG_DMAC_DSAR(chan) = CPHYSADDR(phyaddr);
		REG_DMAC_DTAR(chan) = CPHYSADDR(SPDIF_FIFO);
		REG_DMAC_DTCR(chan) = (count / 32);
		REG_DMAC_DCMD(chan) = DMAC_DCMD_SAI | DMAC_DCMD_SWDH_32 | DMAC_DCMD_DWDH_32 | DMAC_DCMD_DS_32BYTE | DMAC_DCMD_TIE;
...
static irqreturn_t jz_spdif_replay_dma_irq(int irq, void *dev_id)
{
	...
	if (__dmac_channel_transmit_end_detected(dma)) {
		...
		if(pop_dma_flag == 1) {
			pop_dma_flag = 0;
			avialable_trans++;
			wake_up_interruptible(&trans_queue);
		}
	}
...
static int jz_spdif_write_mixdev(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
	char    buf_byte = 0;
	int     val;
...
static int install_dma_intr_handler(int chan, char *name,
		irqreturn_t (*irqhandler)(int, void *),void *data) {

	return request_irq(IRQ_DMA_0 + chan, irqhandler, IRQF_DISABLED, name, data);
}
...
#else
	controller->dma1 = 7;
	if (install_dma_intr_handler(7, "audio dac", jz_spdif_replay_dma_irq, controller) < 0) {
...
#define I2SCDR  0xb0000060
#define REG_I2SCDR      REG32(I2SCDR)
static int __init init_jz_spdif(void)
{
	...
	while((REG_I2SCDR & (1 << 28)))
	{
		printk("wait i2S BUSY is 0...\n");
		printk("I2SCDR = 0x%08x\n", REG_I2SCDR);
	}
	printk("I2SCDR = 0x%08x\n", REG_I2SCDR);
	...
	init_waitqueue_head(&trans_queue);
	avialable_trans = 0;
...
static int jz_audio_open(struct inode *inode, struct file *file)
{
	...
	if (file->f_mode & FMODE_WRITE) {
		...
		__spdif_set_transmit_trigger(32);//32
...
static ssize_t jz_audio_write(struct file *file, const char *buffer, size_t count, loff_t *ppos)
{
	...
	wait_event_interruptible(trans_queue, (avialable_trans >= 1));
