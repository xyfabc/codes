	...
	do {
		...
		if (!node) {
			...
			if (!block)
				...
				replay_ring.someone_waiting = 1;
				/* the best time is (frag_size / sample_rate) + 1s
				   here, we hope the user do not set a very large frag_size
				 */
				ret = wait_event_interruptible_timeout(replay_ring.wait_queue,
						(replay_ring.someone_waiting == 0),
						3 * HZ);
...
int jz_audio_dma_pull(u8 *data /* user */, int len, int block, int first_read) {
...
static int install_dma_intr_handler(int chan, char *name, 
		...
		void *data) {
	return request_irq(IRQ_DMA_0 + chan, irqhandler, 0,
					name, data);
}
